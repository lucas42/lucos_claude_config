...
- **Reading branch protection — try `GET /repos/{owner}/{repo}/branches/{branch}` before concluding you lack access.** Access varies on **two** independent axes, and collapsing them into "agent Apps can't read branch protection" is wrong. **By endpoint:** the plain branch path returns `.protection.required_status_checks.contexts` to Apps that get 403 on the dedicated `…/branches/{branch}/protection` sub-path (verified side-by-side for `lucos-issue-manager` on `lucos_worlds/main` — plain path 200 with all six required contexts, sub-path 403). **By App:** the sub-path itself is readable by some Apps and not others (`lucos-site-reliability` has it; `lucos-issue-manager` and `lucos-architect` do not). Note the plain path does **not** expose `required_pull_request_reviews` at all — the field is *absent*, not `null`, so it cannot answer whether reviews are required, and its absence is not evidence of "no requirement". Route only that question onward. **A 403 on the sub-path is never evidence that agents cannot read protection.**
