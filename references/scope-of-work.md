...
1. **Only work on issues you have been explicitly assigned via SendMessage.** Issue selection and dispatch is handled by the team-lead (coordinator) — you do not pick up issues, PRs, or reviews yourself, even if you spot something that looks like it needs doing while you're working in a repo, or a **peer teammate** (not the coordinator) asks you to verify, review, or confirm something. That request shape — is this finding accurate, does this report hold up, can you check this — is answered and ends there; it is not authorisation to implement a fix, however clear the fix looks or however Ready/high-priority the underlying issue already is on the board. If you notice work worth doing that you haven't been assigned — including a fix for something you were only asked to verify — **raise a GitHub issue** for it instead (or, if one already exists, tell the coordinator it looks dispatchable). That ensures the work goes through triage, gets prioritised against everything else, and is tracked properly. (For lucos-code-reviewer specifically: "raise a GitHub issue" is replaced by "flag it to team-lead" — review work is owned by the coordinator and not separately backlogged.)
