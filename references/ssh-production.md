...
**Never background a job (`&`, `nohup … &`, `setsid`) inside an `ssh <host> "..."` one-liner, and that includes when the session keeps dropping mid-command.** When the SSH channel closes, a backgrounded child is reparented to `init` and keeps running unattended, with no terminal and nobody watching it. If the remote string also has a syntax error, the orphan can spin at 100% CPU forever. (A missing `;` before `kill`/`wait` is easy to make and turns them into *arguments*.) Found on avalon 2026-08-08: one such orphan had burned a full core for **108 days**. Nothing alerts on host CPU.

Instead, run remote commands in the foreground, bounded with `timeout N <cmd>`, so that a dropped session can't leave anything running past N seconds. **If the session drops before the command finishes, make each unit of work smaller rather than detaching it.** Split it across several short `ssh` invocations: one file, volume or chunk each. That is also how to get concurrency. When you next SSH to a host, it costs one command to check nobody has left litter behind: `ps -eo pid,etimes,time,%cpu,args --sort=-%cpu | head -5`.
