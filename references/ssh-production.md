...
**Never background a job (`&`) inside an `ssh <host> "..."` one-liner.** When the SSH channel closes, a backgrounded child is reparented to `init` and keeps running unattended — with no terminal, nobody watching it, and no timeout. If the surrounding shell fragment also has a syntax error (very easy in a multi-command remote string: a missing `;` or newline before `kill`/`wait` silently turns them into *arguments*), the orphan can spin at 100% CPU forever. Found on avalon 2026-08-08: a `docker run … & sleep 3 kill %1 … wait` one-liner from 2026-04-22 had been burning a full core — 25% of a 4-core host — for **108 days**, consuming 107 days of CPU time. Nothing alerts on this; host CPU is not an SRE monitoring check and a `bash` process is not a container crash. Instead: run remote commands in the foreground and bound them with `timeout N <cmd>`. If you genuinely need concurrency, run several separate `ssh` invocations rather than backgrounding inside one. When you next SSH to a host, it costs one command to check nobody has left litter behind: `ps -eo pid,etimes,time,%cpu,args --sort=-%cpu | head -5`.

**Bracket the first character of any `pgrep -f` / `pkill -f` pattern in a remote command**, e.g. `pgrep -f '[s]ha256sum -c'`. `ssh host "…"` runs your string under a remote shell whose own command line contains the pattern. An unbracketed pattern therefore always matches that shell: `pgrep` reports a finished job as still running, and `pkill` kills your own session. Prefer killing by a PID you captured.
