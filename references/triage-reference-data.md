...
**That inaccessibility is silent on reads, which makes other agents' board claims unreliable.** A `projectItems` GraphQL query run under a persona App token returns an **empty connection rather than an error** for a fully-boarded issue — so any teammate reporting "issue N isn't boarded" / "these issues were never triaged" is reporting a result their credentials could not have produced a positive answer to. **Never accept or act on another agent's assertion about board state; re-query with `gh-projects` yourself before agreeing, correcting anyone, or letting it change an artifact.** The same applies to labels: workflow state moved off labels onto board fields, so an empty label list is the *expected* steady state for correctly-triaged work, not evidence of anything. (Confirmed 2026-07-31: `lucos-code-reviewer` blocked the merge of an incident report on a finding that four correctly-boarded, correctly-fielded issues were "not boarded at all" — an App-token false negative that nearly caused an accurate permanent record to be degraded to satisfy it.)
...
- Complete all four steps (add, set fields, position) as a single unit before moving to the next issue. **The Priority *field* does NOT affect board ORDER.** `addProjectV2ItemById` appends every new item to the **bottom**, and it stays there until `updateProjectV2ItemPosition` moves it — an item can show `Priority = High` and sit dead last. So "fields are set" is **not** "done": a board action without a position call leaves strategic-tier-1 work stranded at the bottom of Ready (observed 2026-06-26 — aithne migration tickets boarded High but left at positions 26–28, below Medium/Low items, until lucas42 flagged it). If you script boarding as a reusable helper, the position call MUST be in it. **After boarding any item (or batch), re-query the Ready column in board order and confirm your High/strategic items are actually near the top — not merely tagged High.**
...
- **Keep GraphQL query strings multi-line as written — do NOT hand-compress to a single line.** A dropped/miscounted brace in a compressed query produces opaque `gh: Expected NAME, actual: (none)` parse errors, and inside a Done-sweep delete loop the error tokens get silently fed back as item IDs (`Could not resolve to a node with the global id of 'NAME,'`). Copy the multi-line forms above verbatim; don't collapse them.
