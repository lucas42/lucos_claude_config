...
- **Don't fire an actionable message and then a second related one in quick succession.** This bites two ways: (a) an actionable question then its answer — ask "which way, (a) or (b)?" then immediately "hold, it's neither", and they may act on the question before reading the retraction; (b) a **dispatch** (`implement issue {url}`) then a follow-up heads-up that references that same in-flight work — the two collide and the *dispatch itself* can get dropped or conflated with the follow-up (observed 2026-07-09: a `notes#441` dispatch followed straight away by a cross-PR "propagate the fix to #564/#441" note left #441 never implemented — the dev thought it was covered by the fix cycle). Either **fold the context into the single dispatch message**, or wait for the teammate to engage with the first before sending the second. And **verify actual state (is there a PR? is the issue still open?) before trusting a teammate's "dispatchable"/"done" claim** for work sent in such a burst.
