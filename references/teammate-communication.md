...
**The same obligation runs forwards: phrase your own derived claims so the recipient can re-derive them.** This bites hardest on **operational predictions** — when something will fire, red, expire, or need action. State them **relative to their triggering event**, not as an absolute date or weekday: "~48h after the bump PR opens" survives a slipped trigger, a changed schedule, and a reader three weeks later; "Sunday" is wrong in all three cases and *silently* so, because the derivation that would expose the error has been discarded. Where an absolute date genuinely helps, give both and show the work — "~48h after it opens, so Monday 2026-08-03 on the current schedule" — which is checkable rather than asserted. The same applies to any figure you computed rather than observed (a count, a duration, a cost): carry the basis with the number, so a wrong basis is visible instead of inheriting the number's air of precision. (Confirmed 2026-07-31: an SRE prediction of a `stale-dependabot-prs` red was relayed as "Sunday" when the arithmetic gave Monday. The durable artifacts had used the relative form and were correct; only the chat message — the surface that lands in someone's head without being re-derived — carried the error.)

**Round-trip every identifier you publish, in the same turn you publish it.** This is a *different* failure from everything above and needs its own mechanism: the rules in this section target claims that **outran** their evidence, where the fix is "go and get evidence". A mistyped SHA, PR number, issue reference or URL is a claim that **misquotes** evidence already in hand — verification discipline cannot catch it, because you were not being careless with the evidence, only with the copying. What identifiers have that inferences don't is that they are **cheaply checkable by machine**: `git log --oneline -1 <sha>`, a `gh-as-agent` fetch on the issue/PR number, a `curl -o /dev/null -w '%{http_code}'` on the URL. So the rule is not "be careful copying" (unenforceable, and you already were) — it is **anything shaped like an identifier gets resolved before it is sent**, including identifiers *you produced yourself*, which the verify-a-teammate's-identifier rule does not cover. Fails loudly, costs a second, and does not depend on reading back accurately — the one thing demonstrably not to be trusted here. This generalises the narrower existing habit of fetching a PR body back after creating it to confirm what shipped is what you meant. (Confirmed 2026-07-31: the coordinator quoted a commit as `1b0b30f` when it was `29054ee`, in a message where the identifier was the entire payload; caught by the recipient with a single `git log`, seconds after the coordinator had written a paragraph about making derivations checkable.)
