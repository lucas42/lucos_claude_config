...
- **Never use `#N` for a Dependabot, CodeQL, or secret-scanning alert number** — not in GitHub comments, issue/PR bodies or titles, commit messages, or in reports/messages to teammates. Alert numbering is a completely separate space from issue/PR numbering, and **fully qualifying it as `owner/repo#N` does not fix this** — the estate-wide cross-repo-reference rule (qualify with `owner/repo#N`) exists to disambiguate *which repo* an issue/PR number belongs to; it says nothing about whether `#N` is the right syntax for an alert at all, and a qualified `owner/repo#N` for an alert number reads as fully compliant while still resolving to whatever real issue/PR happens to hold that number in that repo — silently cross-linking the wrong thing with no broken-link signal to catch it. Always use the CVE or GHSA identifier instead (e.g. `CVE-2026-0540`, `GHSA-v2wj-7wpq-c8vv`) — GitHub auto-links these and they can't collide with an issue/PR number. If neither exists, use the full alert URL, or spell it out unambiguously in prose ("Dependabot alert 61", never bare "#61" or "repo#61").

**This applies just as much to a later reference back to an alert as to the first time you mention it** — closing it out, confirming it cleared, summarising it in a follow-up — not only to drafting the original issue or fix PR. And it applies to your own working notes and memory files, not only GitHub-facing text: if a note records the alert as `repo#N`, that's the label you'll reach for next time you read the note and write about it again, so the bad form re-enters a future draft even after the GitHub-facing instance was fixed. Write alert numbers into your own notes the same way — CVE/GHSA id, full URL, or unambiguous prose — not as `#N`.
