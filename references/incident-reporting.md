...
3. **Fill in the template** using information gathered from the issue and its linked PRs:
   ...
   - **Follow-up actions**: Link to any open issues raised as a result. If any follow-up actions do not yet have a tracked issue, raise one and link it in the table. **The SRE writing the incident report is the designated owner of filing all follow-up issues.** Other agents must not independently file follow-up issues from the same incident — this causes duplicates. If another agent identifies a follow-up action, they should message the SRE or the issue manager rather than creating the issue themselves. **A follow-up that needs lucas42's go/no-go is still filed as a full-information issue and routed to him for approval (board: Awaiting Decision, not Ready) — do NOT leave it as an unfiled "deferred to lucas42" line in the follow-up table.** He cannot make a well-informed decision off a one-line disposition, and an undecided-but-unfiled follow-up strands untracked. The issue carries the full context (incident, root cause, options + tradeoffs); the coordinator boards it Awaiting Decision and it becomes Ready only on his approval.
...
1. **Re-verify the live state of every ticket the report references, immediately before marking ready.** A report drafted over hours will cite issues that were open when written and have since been closed — often *by the very follow-ups the report prompted*. Sweeping the prose for stale claims (`TBD`, superseded numbers, "still outstanding") is a different check and does not catch this: the wording is fine, the world moved. Fetch each referenced issue/PR's `state` and fix any row that has changed, **and fix every place that describes it** — the Follow-up Actions table, any prose still calling a shipped fix a "proposal", and the PR body's own copy of the table, which a reviewer reading the diff has no reason to check. (2026-08-17: `lucos_docker_health#108` was listed Open in all three surfaces; it had closed 35 minutes before the report's previous commit, caught by the code-reviewer rather than by my own sweep.)

2. **Tick the outstanding-items checkboxes in the PR body** (the TBD list from Step 2) now that each is done, *then* **mark the draft PR ready for review** (GraphQL `markPullRequestReadyForReview` — see `references/github-workflow.md` for the mutation and node-id lookup). The code-reviewer verifies those checkboxes on unsupervised repos and an unchecked gate can hold the approval, so don't leave them unticked even when the body text demonstrably satisfies them.
3. **Drive the code-reviewer review loop to approval and merge**, per `pr-review-loop.md` — message `lucos-code-reviewer` and don't stop at "marked ready". On `lucos` (unsupervised) reviewer approval triggers auto-merge.

### ⚠️ On `lucos`, requesting review IS requesting merge — the branch is frozen from that moment
...
git show origin/main:<file> | grep -c '<text you just added>'   # plus a positive control
```

If something did miss the merge, cherry-pick it onto a fresh branch off `origin/main` and open a follow-up PR referencing the original.

> 2026-08-08, **twice within one hour.** First: a corrected rollback command pushed ~2 minutes after lucas42/lucos#281 auto-merged, leaving the published report telling readers to "restore from the recorded config" while the obvious implementation of that fails — in the recovery step of a destructive operation (fixed by lucas42/lucos#284). Then, while lucas42/lucos#284 was in review, the reviewer's *own* requested edit was pushed to that same branch and missed its merge too (fixed by lucas42/lucos#285). The first version of this rule said "don't mark ready while a thread is live", which did not fire the second time because lucas42/lucos#284 was never a draft. Hence the broader rule above.
