...
3. **Write a thorough issue** that includes:
   ...
   - **Provenance, when the issue did not originate with you.** If lucas42 (or anyone else) asked for this ticket to be raised, reported the symptom, or otherwise originated it, record that on its own line directly under the title heading, in exactly this form:
     ...
     `Originated by: lucas42` — or `Originated by: lucos-issue-manager`, or whoever it actually was. **Name the real originator; the example is not a default.** If nobody asked for the ticket to be raised, the line is simply absent, which is the normal case for anything an agent finds itself.

     Use the fixed wording — it exists to be greppable, so a later question like "show me every open ticket that came from me" has a reliable answer. **This is about who the ticket came FROM, not who supplied facts or decisions during its life.** In particular, **work you were asked to do is not a ticket you were asked to file**: a finding that surfaces while carrying out someone's request originated with you, not with them. The tell is reaching for this line because the investigation was commissioned rather than because the ticket was. A ticket an agent found itself does not become lucas42-originated because he later answered a design question on it, picked an option at triage, or is quoted in the body; those are ordinary participation and must not carry this line. Conversely, a ticket he reported in chat and asked you to file IS his in origin even though the GitHub author is a bot — and without this line there is no way to tell, because the author field says otherwise and nothing else in the ticket records it.
...
4. **Create the issue** using `gh-as-agent`. There are two patterns; **pick the right one for your body content**:
   ...
   **This is a warning to eyeball, never a gate — do not "fix" it into a hard block.** It has known, legitimate hits: a six-digit hex colour (`#123456`) in a UX ticket; any fenced block that deliberately shows a backticked reference while documenting this very rule (this file trips its own check); and — commonest of the three — **a line carrying two code spans with a bare `#N` in the prose between them**. The pattern cannot tell a closing backtick from an opening one, so it matches from the first span's *closing* delimiter to the second's *opening* one. That case fires on **correctly written** references, which means the more disciplined the author the likelier the hit, and a reader who trusts the flag without inspecting could "fix" a bare reference into a backticked one — the very defect this exists to catch, inverted. Blocking on any of these would stop correct bodies being filed.

   **Do not narrow the pattern to eliminate that.** Forbidding whitespace inside the span — `` `[^`[:space:]]*#[0-9]+[^`[:space:]]*` `` — removes the two-span hit cleanly, and also stops matching a whole phrase backticked around a real reference (`` `Blocked on lucas42/lucos#292` ``), which is a genuine defect that fails to autolink exactly the same way. This check exists to catch a **silent** failure, so recall is worth more than precision here: a noisy hit costs seconds of inspection, a missed one strands a ticket nobody is looking at. Both patterns were run against all four cases before this one was chosen.
   ...
   **The mechanism behind the silence:** `skills/dispatch/check-dependent` strips code spans before matching, deliberately, to mirror the autolinker. So a backticked blocker leaves the ticket stranded in Blocked with its prerequisite already merged, while the body reads perfectly to a human and states its dependency unambiguously.
