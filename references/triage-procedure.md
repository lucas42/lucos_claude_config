...
**When a brand-new system IS green-lit, its ADR goes in the new system's own repo as that repo's ADR-0001 — not as an ADR in the `lucos` repo.** Create (or have `lucos-system-administrator` create) the new empty repo first, then have the architect write the design up as `ADR-0001` there. ADRs in the `lucos` repo are for estate-wide / cross-system decisions; a new system's founding design belongs with the system it establishes, where it's easier to follow. (ADRs for changes to *existing* systems still follow each system's established location.)
