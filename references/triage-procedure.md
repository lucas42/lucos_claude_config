...
This check applies regardless of whether the originating agent is doing routine ops checks or one-off investigation. If the raising agent's own framing already includes a phrase like "the auto-PR will appear when upstream patches", treat that framing as the signal that no separate issue is warranted.
...
- **Multiple options with no decision is a hard blocker for `agent-approved`.** If an issue lists two or more implementation options without indicating which to take, it is NOT ready to implement — a developer picking it up has no idea what to build. Either: (a) consult the relevant subject-matter expert agent (architect for design choices, SRE for reliability/observability trade-offs, security for security-sensitive choices, **sysadmin for infrastructure/mirror/deployment/Docker decisions**) to get a recommendation and post it as a comment, then re-assess; or (b) if only lucas42 can decide, mark it `needs-refining` + `status:awaiting-decision` + `owner:lucas42`. Do not let "the options seem obvious" or "any of them would work" justify skipping this step — the implementer needs a single agreed direction, not a menu. **An agent's recommendation is not a decision.** After getting a recommendation, re-assess: if lucas42 still needs to confirm the choice, mark `needs-refining` + `status:awaiting-decision` + `owner:lucas42`. Only mark `agent-approved` if the recommendation is unambiguous and no further confirmation from lucas42 is needed.
...
- Does it **introduce a new data model / entity type, or make a significant change to an existing one** (e.g. new Django model, new knowledge-base entity class, schema migration that changes relationships)? → consult `lucos-architect`.
...
1. **Update the issue body to reflect the current agreed scope and approach.** Before marking agent-approved, ensure the body describes what is to be built — incorporating any decisions, constraints, or revised approaches from the comments. The implementing agent reads the body, not the comment history. If the body is stale, the agent will implement the wrong thing. This is not optional.
2. Add the label `agent-approved` to the issue.
3. Remove the label `needs-refining` if it is present.
4. Remove any `status:*` and review-phase `owner:*` labels.
5. Assign an **implementation owner** label (see "Implementation Assignment" below).
