...
- **Check reactions on the ISSUE ITSELF *and* on every comment**, especially +1 from `lucas42` — a +1 on a design proposal counts as approval (see "Reactions as approval" below). Two separate endpoints, and one says nothing about the other: `/issues/{n}/reactions` for the body, `/issues/comments/{id}/reactions` for each comment. **The body is where the proposal usually lives on an agent-filed ticket** (a spike follow-up, a written-up recommendation), so that is where lucas42 reacts — a comments-only check finds nothing and wrongly reports it as still-pending. Do not skip an issue just because the last commenter is `lucos-issue-manager[bot]` — lucas42 may have reacted without writing a new comment.
...
- **A body or title still describing a *superseded* approach is a hard blocker for setting Status = Ready** — the mirror of the rule above. When a thread walks back or replaces its original approach, the agreed design lands in a **comment** while the body goes on advertising the rejected one. This passes an options-check cleanly (the body names one concrete approach; nothing reads as deferred), which is exactly why it needs its own gate. Since `/dispatch` sends the URL alone and the body is what the implementer reads first, a "Proposed fix" naming a rejected approach is *worse* than an under-specified one — it reads as actionable and gets built. Ask: *did the approach change after the body was written?* If yes, treat the body as stale until checked. Have the **owning agent** fold their agreed design back in and retitle (it's their design — don't paraphrase it yourself and risk drift), and resolve any artifact of the dead approach (an open PR implementing it also trips `/dispatch`'s existing-PR guardrail). Applying the fix in Step 3 is not optional cleanup — until it's done, the issue is not Ready.
...
**Target codebase**: If the issue doesn't yet know which repo the work belongs in, that is an unresolved architectural question — set Status = Needs Analysis + Owner = lucos-architect on the project board, even if the rest of the issue is well-specified.

**New-system / new-project proposals stay in Ideation until lucas42 explicitly asks for a design.** When lucas42 files a ticket proposing a *brand-new system or project* (not a change to an existing system), do NOT route it to the architect for design framing — even if it reads as well-specified. Set Status = Ideation + Owner = lucas42 and leave it parked; a new-system proposal is exploratory by default, and lucas42 decides when it graduates to a committed design. Commission architect framing / an ADR only when he explicitly green-lights it ("design this", "let's build this", "commission the design"). **Answering an architect's clarifying questions is NOT that green light** — lucas42 engaging with a design conversation while the ticket sits in Ideation does not authorise proceeding to an ADR or a build. This is the new-system exception to the inline-consultation default, which applies to changes on *existing* systems.
...
- **Do NOT accept "parallel unit-testable" or "can be written and tested independently against fixtures" framings as grounds for Ready.** Status = Ready means the work can be implemented AND merged to a working end-to-end state without waiting on another open ticket. If any cross-repo or in-repo dependency must close before this change can be merged to a working production state, the issue is **Blocked** — full stop. The architect (or any raiser) may include language like "this can be implemented in parallel using fixture data, end-to-end validation waits" as an implementation note in the body, but that does not override the Blocked decision at triage. Unit tests against fixtures do not establish that the integration works in production. (Lessons from 2026-05-18: `lucos_arachne#539` and `lucos_search_component#165` were both initially triaged as Ready under this framing despite having open cross-repo dependencies.)
  ...
  **Qualifier — this rule is about tickets that SHIP CODE, and does not transfer to a design-document ticket.** The rule's force comes from "merged to a working production state": code merged ahead of its dependency is unverifiable in production. A ticket whose entire deliverable is a document (an ADR, a spec, a written decision) has no such state — its verification is lucas42's sign-off on the *reasoning*, which does not require the dependency's code to exist. So a design ticket that builds on another ticket's **recorded decision** is **Ready**, even while that other ticket's implementation is open; it is Blocked only if the decision it rests on is itself still unsettled. Ask which of the two the dependency actually supplies: a *decision* (design ticket is Ready) or *code the deliverable must run against* (Blocked). Two traps to check: (a) a raiser may state a bare "Blocked on #Y" over reasoning that is really about not shipping code before #Y — read the argument, not the label; (b) verify the dependency's actual state rather than assuming an in-flight PR exists, since "its shape could shift under us" is only a real objection if something is genuinely in review. Reaching for Blocked here feels conservative but is simply wrong, and it defers work that could have proceeded.
...
**CHECKPOINT — read the WHOLE comment thread before transitioning, never a filtered view.** When you fetch a ticket's comments to assess a transition, read *every* comment from *every* author in full — never an author-filtered subset (e.g. "just the architect's design comment") and never a single comment in isolation. A `lucas42` comment interleaved between agent comments is invisible to an author-filtered fetch; skipping it means transitioning the ticket past a question or decision he already posted (e.g. moving to Awaiting Decision over the top of an open "isn't this a duplicate of #X?" question he asked an hour earlier). If you fetched comments with a `select(.user.login==...)` filter to verify one teammate's input, that is NOT sufficient to transition — re-fetch the full thread first.
...
**STOP — manual-lucas42-step checkpoint.** Before setting Status = Ready, assess whether *implementing or deploying* this issue will require a manual step only lucas42 can perform — e.g. writing a production (non-development) credential, changing a GitHub App's permissions, minting/rotating a production machine key, a registrar / DNS / third-party-vendor action, or anything outside the agents' access. If so, flag it **now, at triage** — not at dispatch/PR/merge time. Record it in the **issue body** (a "⚠️ Requires a manual step from lucas42: …" note near the top, so it's durable and visible whenever the issue is opened) **and** call it out as a distinct line in the lucas42 triage summary, **even when the issue's Owner is an implementer rather than lucas42**. lucas42 often dispatches issues in the background and needs to know in advance whether his presence is required. If the manual step is a hard prerequisite the work can't proceed or deploy without, treat the issue as **Blocked** on it rather than Ready.
...
"Another agent" includes both **specialists** (architect, SRE, security, UX) being consulted for design input AND **the developer (or any other implementation agent) when they need to do pre-implementation verification, research, or option-choosing at Needs Analysis stage** before the issue can move to Ready. Don't restrict this section to specialists in your head — `Owner = lucos-developer` at `Status = Needs Analysis` means the developer has a *verification/analysis* task, not an implementation task, and they need the same explicit SendMessage handoff as any specialist consultation.
...
**When lucas42's comment is already on the ticket, do NOT quote, paraphrase, or re-frame it in the SendMessage — point the agent at the ticket and let them read it first-hand.** The agent reads the ticket as step one and can read lucas42's words better than you can relay them; a quote you add is redundant and a second copy that can drift from — or be confabulated against — the original. The SendMessage carries only the issue URL, the project-board context (Status, Owner), and a high-level pointer ("lucas42 has commented again — read his latest comment on the ticket and respond there"). Quote only content NOT visible to the agent (e.g. a chat-only decision), pasted from its source in the same turn, never from memory. **Do not augment with your own option list, suggested approach, or "options I see" framing.** lucas42 deliberately leaves design questions problem-oriented to keep the consulting agent's mind open to approaches neither of you have thought of. Even when you can think of options, listing them biases the agent toward your framing and away from approaches outside your enumeration — and "more obvious" defaults dominate. Quote lucas42's words; add only the project-board context (Status, Owner) and the high-level ask ("please propose how to address this"). The agent has the domain expertise to enumerate options on their own; that's their job, not yours. If you genuinely don't understand lucas42's question well enough to relay it, ask him to clarify — don't paraphrase it for the specialist with your own framing.
...
**Verbatim quoting belongs in SendMessage, not in the triage-decision comment on the ticket.** The audiences are different. The architect (or any other reader of the GitHub thread) can already see lucas42's comment directly above yours; quoting it back is noise that doubles the visual weight without adding information. The triage-decision comment on the ticket exists only to (a) clear the re-triage flag and (b) record the board-state change — keep it short ("Re-triaged: Status = Needs Analysis, Owner = lucos-architect. Routing back to address lucas42's latest comment."). Save the verbatim quote for the teammate's inbox, where they don't see the ticket thread.

**That comment is REQUIRED on every ticket you triage — including, especially, one you deliberately park.** Board fields are invisible from the ticket: someone reading a triaged issue with no comment sees an issue nobody has looked at, and cannot distinguish "owned, analysed, consciously not started yet" from "missed". The riskier omission is therefore the *deliberate* one — when the triage outcome is "correct owner set, deliberately not nudging yet", the reasoning exists only in your head and in a pass-summary that scrolls away, so the ticket carries no evidence any decision was made at all. Record the decision and its reason on the ticket ("Owner = X, deliberately not started yet because Y — will surface on the next pass"), not just in the pass summary. If you catch yourself self-attesting "deliberately holding" at Step 4.5, that attestation belongs on the ticket too. (Lesson 2026-07-19: `lucos_photos#477` was boarded Needs Analysis / Medium / `lucos-developer` and deliberately not nudged, but got no comment while five sibling tickets triaged in the same pass did — lucas42 reasonably read it as untriaged.)
...
1. Set Status = Awaiting Decision (if lucas42 must choose between options or provide a decision), Status = Needs Analysis (if an agent owes design/analysis work first — set Owner to that agent), or Status = Ideation (if scope is genuinely vague and parked with no one assigned). Remove any legacy workflow labels.
...
- if your comment surfaces questions another specialist should weigh in on → set Status = Needs Analysis + Owner = lucos-\<specialist\> on the project board
...
For issues with Status = Needs Analysis, Ideation, or Awaiting Decision from a previous session, detect completed agent work: if an agent's comment is the most recent activity, treat it as their completed input — transition to Status = Ready if uncontroversial, or Status = Awaiting Decision + Owner = lucas42 if it still needs sign-off.

**Needs Analysis means the assigned agent has analysis still _outstanding_ — move the item on the moment that input lands; don't wait for a future triage pass.** When an agent posts their analysis/design turn (or reports back to you that they have), their input is complete *for now* and the item must leave Needs Analysis the same turn, routed to whoever must act next. **A design dialogue that will continue is NOT grounds to keep it in Needs Analysis:** once the agent has posted their turn and the ball is back with lucas42, move it to his court — Ideation if the next step is open-ended exploration, Awaiting Decision if a concrete proposal is on the table for him to accept/reject. Leaving it Needs Analysis + Owner = agent after the agent has already replied makes a completed turn look like outstanding work — that is the staleness that strands items between passes.

**Reactions as approval.** A +1 reaction from `lucas42` on **the issue body or on a comment** is approval of the recommendations in it. Most relevant when an agent has posted a design proposal: a +1 on a design proposal or implementation plan transitions the issue to Status = Ready (assuming no other blocking questions); a +1 on a comment listing multiple options with a recommendation counts as agreement with the recommended option.

**The body counts as much as a comment, and is the likelier place.** When an agent *files a ticket whose body is the proposal* — a spike's follow-up, a written-up design, a recommendation — there is no comment carrying the design, so lucas42's `+1` lands on the issue itself. Checking only `/issues/comments/{id}/reactions` returns nothing and the ticket looks unanswered. Always fetch `/issues/{n}/reactions` too.
