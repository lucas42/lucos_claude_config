...
- **Multiple options with no decision is a hard blocker for `agent-approved`.** An issue listing two or more options without indicating which is NOT ready — the implementer has no idea what to build. Either (a) consult a specialist for a recommendation (see [`specialist-routing.md`](specialist-routing.md)), post it, re-assess; or (b) if only lucas42 can decide, mark `needs-refining` + `status:awaiting-decision` + `owner:lucas42`. "Any of them would work" does not justify skipping — the implementer needs one agreed direction, not a menu.
