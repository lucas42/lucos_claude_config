...
| `checks` | object | 1 | Health checks: each value has `ok` (bool), `techDetail` (string), optional `debug` (string), optional `failThreshold` (integer, default 1 — number of consecutive failures required before monitoring alerts; use for known-flappy checks), optional `dependsOn` (system ID string — suppresses alerts when that system is being deployed) |
