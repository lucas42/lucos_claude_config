...
4. **Boundary — this rule is ONLY about cases where `Closes`-automation could do the job.** When a PR deliberately uses `Refs` (not `Closes`) because it only delivers part of a multi-criterion issue, GitHub automation genuinely *cannot* close that issue — so a manual close is required once all criteria are met. In that case the **coordinator closes it directly** (`state_reason: completed`), and **teammates do NOT close issues even when explicitly asked** (per lucos_claude_config#71). Don't delegate the close to the owning teammate and don't ping-pong — just close it yourself once the completion summary is posted. This is not "shifting work onto the coordinator"; it's the coordinator's established job where automation can't apply.
