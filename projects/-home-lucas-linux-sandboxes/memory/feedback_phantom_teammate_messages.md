...
9. **New variant 2026-05-30 (artist-migration verification session): phantom *tool output*, not a phantom message.** Primed by an SRE process-note warning that the session's tooling was degraded ("confabulated narration spliced into shell output"), I narrated *in my own reasoning* that my `gh-projects`/Bash calls had returned garbled/confabulated output — and began drafting a false "the environment is compromised, I can't verify board state" alarm to lucas42 — when the actual `<result>` blocks were clean, well-formed JSON. Caught before sending only because the next real tool result was clean and contradicted my narration. Lessons: (a) the phantom failure mode applies to **tool-result content**, not just inbound messages; (b) a teammate's warning that *their* environment was unstable is **not** evidence that *my* tool results are — don't let it prime me into "seeing" corruption that isn't in the `<result>` block; (c) the priming trigger ("expect garbled output") is itself the tell. Verify against the actual `<result>`/`<output>` block, never against my own narration of what it said.

10. **Precise mechanism (confirmed 2026-05-30 via lucos_agent_coding_sandbox#85 / lucos#155 investigation): confabulation-into-the-empty-gap, NOT injection.** When a tool returns empty/delayed output (e.g. under memory-pressure batching, where the real result arrives several turns later), the model fills the gap with a *plausible expected result* and then treats its own guess as an observed fact. This is the single root mechanism behind every variant above — phantom teammate messages, phantom tool output, phantom user prompts, phantom idle pings. The SRE demonstrated it cleanly: under degraded tooling they reported a `sha256sum` "returning prose" and specific `free -h` figures (`4.7Gi available / 0 swap`) — all confabulated; when the real batched output arrived it was 100% correct (`ROUNDTRIP_MATCH=yes`, real hash, real `1.4Gi available / 160Mi swap used`). **Framing that matters:** it is *"the agent confabulates results into empty/late tool returns"*, NOT *"the harness injects fabricated text into results"* — there is strong evidence against the latter (every *real* result was correct) and none for it. The robust mitigation is procedural and applies to me as much as any teammate: **treat any empty / blank / "(no output)" / late tool result as `unknown`, never as data — re-run or wait for the real result before asserting anything based on it.** An empty result is the highest-risk moment for confabulation; that is exactly when to slow down, not fill in.
