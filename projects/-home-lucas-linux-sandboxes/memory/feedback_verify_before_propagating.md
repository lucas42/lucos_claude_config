...
3. **Second-hand attributions ("agent X says agent Y flagged/requires Z")** relayed onward — especially a security/blocking requirement I forward to an implementer to act on. The relaying agent may have misread the third agent's actual position. **Attribute it to the reporter, not the source** ("per the code-reviewer, security flagged Z — confirm with security"), and for a blocking/security claim verify with the *named source* before stating "Y flagged Z" as fact. (Incident 3, 2026-06-16: the code-reviewer reported "security flagged an IDOR concern on lucos_aithne#123 revoke handlers"; I forwarded it to the developer as "flagged by security." Security had said the *opposite* — `aithne:admin` = full admin access, no per-credential ownership check needed. The "confirm with security if ambiguous" hedge I added is what caught it; without it the developer might have built an unneeded check.)
