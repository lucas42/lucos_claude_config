...
- **Bootstrap first-passkey enrolment → ticketed as #48** (Needs Analysis / architect / Medium). lucas42 has no pre-existing plan + wants it documented. Routed to architect to design (DB invite insert / script / one-time bootstrap-enrol path) + document (runbook/ADR), consulting security (must not reopen the locked-out property) + addressing the permanent-grant runbook note. Architect to raise an impl ticket if code's needed. **Gates lucas42 actually administering aithne.**
