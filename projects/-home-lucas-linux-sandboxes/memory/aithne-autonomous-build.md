...
- **OPEN — for lucas42:** how does the bootstrap admin enrol their FIRST passkey? Principal+grant exist but no passkey → can't log in → can't self-invite. Needs a plan (direct DB invite insert / one-time provisioning script / a one-time bootstrap-enrol path). Surfaced to lucas42 for direction; security offered to help design it. **This gates lucas42 actually using aithne.**
