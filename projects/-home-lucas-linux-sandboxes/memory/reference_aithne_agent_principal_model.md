...
2. **Grant the scope the resource checks — `/admin/grants`.** Per **ADR-0001 §6**, a non-human principal's resource MUST gate on a **scope** ("the only safe lever for non-human principals"; "never gate a sensitive action on bare 'is there a valid session?'"; a zero-grant session should "do almost nothing"). So the grant is the **norm**, and "two actions" (provision + grant) is the expected cutover. A resource gating on `principal_class` alone (no scope) is an ADR-0001 §6 **divergence**, NOT a sanctioned "coarse start" — the arachne `/mcp` canary shipped that way and was flagged (2026-06-16); don't replicate it for the other consumers.
