...
- **On an unsupervised repo, arrange the specialist review BEFORE the PR is opened — afterwards is usually too late.** There, the code-reviewer's approval merges immediately, so open→approve→merge can be ~2 minutes; "wait until the specialist is free, then ask" silently becomes "never". Waiting to avoid competing messages is the exact reasoning that loses the window. If a specialist read is wanted, tell the implementer to request it as part of opening the PR, or accept that it will be a post-merge review of merged code and say so. Recording the commitment on the ticket does not preserve it — only sequencing it before the PR does.
