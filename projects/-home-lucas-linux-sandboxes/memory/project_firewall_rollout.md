...
3. **mDNS `.local` severed** — `lucos_firewall#16` → #17 (ADR-0007 Amendment 3). Base-allow carve-out for link-local mDNS (`224.0.0.251`/`ff02::fb`). Durable fix = aurora onto router-served unicast DNS (`aurora.lan`), **completed + closed 2026-06-08** (`lucos_backups#306`; needed manual volume-recreate per the Compose foot-gun below).
...
- **Compose foot-gun (SRE-flagged, hit TWICE — worth remembering):** Docker Compose **silently reuses a pre-existing named resource (network OR volume) and does NOT apply changed config on redeploy**. A plain redeploy will not reconcile it — you must force-recreate (`docker network rm` / `docker volume rm` + redeploy). Two instances this session: (1) backups#310's `enable_ipv6`/`fd00:3::/64` never took effect because a stale Sept-2024 `lucos_backups_default` IPv4-only network was reused; (2) #306's three xwing NFS volumes (`lucos_private_medlib`, `lucos_static_media_public`, `lucos_media_import_media`) kept `addr=aurora.local` despite merged compose changes to `addr=aurora.lan` — `docker volume inspect <vol> --format '{{.Options.o}}'` reveals the real value (`/proc/mounts` hides it, showing only the resolved IP). Recreating an NFS volume is safe — only the local mount *definition* is recreated; remote data untouched. This bites ANY migration that changes `driver_opts`/network config on an existing named resource. Strong candidate for the architect's `lucos#234` addendum.
