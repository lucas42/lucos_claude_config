...
**Enforce is additionally gated on** the per-host enforce-control mechanism: `lucos_configy#203` (per-host `firewall_enforce` field + `/hosts/{host}` endpoint — Ready, with developer) and `lucos_firewall#9` (firewall reads enforce-mode from configy; Blocked on #203). `DRY_RUN` from creds can't differ per host — flipping a host = setting `firewall_enforce: true` in configy. ADR-0007 amendment recording this = lucos PR #210 (draft, awaiting lucas42's wording sign-off). salvare confirmed by lucas42 to have no public-facing services (base-only rules in enforce expected). The `firewall_enforce` flip to enforce is lucas42's call per host.
