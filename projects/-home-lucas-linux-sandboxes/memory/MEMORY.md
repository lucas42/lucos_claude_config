...
- [ADR Status is a claim, not proof](reference_adr_status_is_a_claim.md) — an ADR's Context describes the status quo it *criticises*; read the Decision and check it shipped before citing
...
- [No secondary sign-off gate](feedback_no_secondary_signoff_gate.md) — /next IS lucas42's sign-off; don't park implementable irreversible tickets in Awaiting Decision, and don't hold reversible doc/config updates as draft PRs gated on his input (they just ship)
...
- [Track out-of-band specialist reviews to merge](feedback_track_out_of_band_specialist_reviews.md) — coordinator-dispatched specialist reviews: arrange BEFORE the PR opens on unsupervised repos (approve→merge is ~2min), then own re-triggering the code-reviewer's final verdict + confirming merge; specialist-APPROVE + code-reviewer-COMMENT is NOT merged (aithne#300 sat 3h16m)
...
- [Close passing audit findings promptly](feedback_close_passing_audit_findings.md) — once a finding's convention is verified passing, close it + board Done immediately; don't leave it for the ≤6h auto-close sweep (2026-07-10)

## Active Projects

- [avalon disk failure](project_avalon_disk_failure.md) — P1 lucos#294 (2026-09-14): single disk died; data rescued to xwing+salvare `~lucos-agent/emergency-backups-2026-09-14/` (README = restore guide); RESOLVED 2026-09-16 — rebuilt on Trixie, all data recovered. Lesson: check the backup exists before deciding it doesn't (incremental snapshots hide under volume-snapshots/). DNS zone expires 2026-10-12 07:09Z. SRE watch stopped — ping SRE when avalon is back; routine triage still owed
