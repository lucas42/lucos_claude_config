...
- [aithne agent-principal model](reference_aithne_agent_principal_model.md) — agent-operating creds live in lucos_agent/**development** (agent-writable, NOT a prod cred). Cutover needs lucas42 to mint the prod machine key (`/admin/machine-keys`, `aithne:admin`-gated); a `/admin/grants` scope grant is needed ONLY if the resource checks a real scope (default-deny is at the resource, NOT token issuance — zero-grant principals still get a scopeless JWT). JWT: `principal_class`/`scopes`/`aud=="l42.eu"`
