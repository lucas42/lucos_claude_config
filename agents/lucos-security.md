...
- **Alert references.** Same rule as the ops-check alert-numbering convention in [`agents/security-ops-checks.md`](security-ops-checks.md) (canonical wording in [`references/github-config.md`](../references/github-config.md) § "GitHub comment conventions") — it applies just as much to an issue you're filing here as to an ops-check fix PR or manifest.
...
Use `--app lucos-security` for all `gh-as-agent` and `git-as-agent` calls. Read [`references/agent-github-identity.md`](../references/agent-github-identity.md) for the heredoc pattern, the `gh api` template-substitution gotcha, the file-backed body workaround, cross-repo issue references, and the `git-as-agent` rules (which you must use for every commit-writing operation, including amends, rebases, and cherry-picks). For `~/.claude` changes specifically, follow the "Committing `~/.claude` changes" section of that reference — **and never run any raw git command there that can write to or discard working-tree state** (`checkout --`/`restore`, `pull`, `stash`, `reset`, `clean`, `rebase`), not even to "double-check" a `commit-claude-main` result. That checkout is shared and routinely holds other agents' uncommitted memory files, so a destructive command there can silently drop work you have no way of knowing exists. Read-only inspection against `origin/main` (`git fetch`, `git log origin/main`, `git diff origin/main -- <file>`) is fine and is the right way to verify on-disk content; `commit-claude-main`'s throwaway-worktree isolation is the only write path. That reference's "Keep instruction-file text lean" section also governs any instruction-file edit you make — rule + why + how-to-apply only, lesson narrative goes in the commit message.
