...
⚠️ **The trigger is per-item failure, not a blank result — so "suspect a uniformly negative result" will not fire.** When a probe runs over N items and each item can fail independently, the failures land on an arbitrary subset and you get a *mix*: some yes, some no. A mix reads as data. Worse, you will then explain the pattern — and a false explanation for a pattern that does not exist is far harder to dislodge than an obvious blank. 2026-08-08, three times in one afternoon: (a) `dig +short AAAA` over 10 domains during packet loss — CNAME'd names need a second resolver round trip, timeouts printed nothing, 2 chains happened to complete, and I published "5 dual-stack / 5 not, an accident of DNS authoring" to a ticket when **all 10** were dual-stack; (b) a `merged:YYYY-MM-01..YYYY-MM-31` census where GitHub silently returns `issueCount: 0` for impossible dates, deleting February and every 30-day month; (c) the same DNS failure hit lucos-architect twice independently. **Three habits, all one line each:** run a **known-negative** control as well as a known-positive (mine returned `NOERROR` for a nonexistent name — impossible, and it condemned the whole run); **reconcile the parts against an independent total** (3,318 summed vs 9,349 unsliced is what exposed the census); and for any DNS claim query the **authoritative** server (`dig +tcp @178.32.218.44` for l42.eu), never a recursive resolver whose timeouts you cannot see. Distrust agreement with your hypothesis most of all — see [[feedback_verify_check_claim_against_underlying_store]].
