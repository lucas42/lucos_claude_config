...
**A probe that finds nothing has not proven nothing is there — and a probe that finds a *plausible pattern* is more dangerous still.** This cuts the opposite way to the root-cause rule below, and is easier to miss because the output looks clean: a silently-broken probe is indistinguishable from a genuine negative — empty output, no matches, a fast "FAIL". Before you clear a container, host, or the estate on "I looked and found nothing", prove your probe can return a *positive* (run it against a known-good case). The four tells — the tool isn't in the image, the probe isn't the app's authenticated call, the process only exists between cron invocations, and **the probe is slower than the event or sits at one phase of it** (a 3s burst in a 60s cycle is a 5% duty cycle: a clean negative is what an under-sampled probe *always* returns) — and the worked examples are in [`sre-operational-defaults.md`](sre-operational-defaults.md) §"Before declaring a root cause — or clearing a component as innocent", check 3. The one question covering all four: *what sampling rate, scope, credentials and window would this probe need to return a positive — and does it have them?* Never `2>/dev/null` the error you need; and when a hand-rolled probe contradicts a green dashboard, suspect the probe.
