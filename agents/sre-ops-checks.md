...
5. **For CI failures: escalate re-runs.** CI failures do NOT self-heal unless something triggers a new run (a commit, a schedule, or a manual re-run). Do not assume self-healing will occur. The SRE CircleCI token is read-only (confirmed — `POST /workflow/{id}/rerun` returns `Permission denied`), so re-runs cannot be triggered locally. You MUST: (a) fetch the most recent failed workflow ID for each affected repo, (b) message `team-lead` requesting the re-runs be routed to `lucos-system-administrator` (which holds the user-scoped CircleCI credentials), and (c) confirm in your report that the re-run request has been escalated and report back the result once triggered. **A CI failure is not resolved until CI is green — documenting the root cause is necessary but not sufficient.**
