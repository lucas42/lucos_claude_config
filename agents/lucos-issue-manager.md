...
**Before reporting on any issue that was actively discussed during the current session** (e.g. consulted an agent, routed to lucas42 earlier), **re-fetch its comments and check reactions** to detect new activity. Do not rely on what you last saw — lucas42 may have replied or added a +1 reaction between triage passes. This applies even if the issue appears in `get-issues-for-triage` with the same labels as before; the labels haven't changed because you haven't changed them yet, but the conversation may have moved on.
