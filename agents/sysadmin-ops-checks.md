...
- **IOPS/load**: `uptime` and `iostat -x 1 3` if available — flag sustained high load. **If load is elevated (vs. this host's recent baseline in `ops-checks.md`) or the 1-min figure is notably higher than 5/15-min, don't just note it and move on** — run `ps aux --sort=-%cpu | head -15` to identify the top CPU consumers. A single process pinned near 100% of one core (visible as `%CPU` close to 100 with a long-running `TIME+`) is a runaway, not noise — flag it explicitly even if overall load isn't yet "high" for the host's core count, since a constant one-core pin can hide inside an otherwise-unremarkable load average for months. This mirrors the disk-usage investigation rule above: don't raise (or dismiss) a load finding without the process-level breakdown behind it. (Prompted by a 108-day orphaned `bash` process on avalon pinned at 99.7% CPU that repeated "elevated load, not urgent" notes in this file never traced to a cause — see `references/ssh-production.md` for the SSH-one-liner prevention rule that stops the underlying cause.)
