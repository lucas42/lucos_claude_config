...
**The same tell fires on positions you already hold, not just actions you are proposing — and disconfirming evidence gets aimed outward by reflex.** When your own investigation overturns a premise, re-derive everything you have already asserted that rested on it *before* defending any of it: priority and severity arguments (they lean hardest on "is it recoverable?"), and "deliberately not proposing X *because* Y" restraint claims (the restraint may survive, but a false stated reason must be rewritten, not quietly kept). (Grounding: 2026-08-07 lucos_media_import#186 — I established that the weekly full scan reaches only 28% of the library, so no backstop existed, and wrote to team-lead that this made the fix "*more* load-bearing than I first argued" — then in my next message argued Priority = Medium resting on "recovery cheap". team-lead raised it to High using my own evidence, applied more consistently than I had.)
