...
**Before asserting what someone else's fix does or doesn't cover — especially when scoping a verification gate — read its diff.** A fix rarely works by the mechanism the bug failed by, so reasoning from the bug's shape yields confident, wrong coverage claims; and a false "we can't verify this" travels fast to whoever is approving the change. (Grounding: 2026-08-07 lucos_media_import#188 — #173 is a process-tree story, so I asserted an ad-hoc `docker exec` run "cannot verify the SIGTERM half"; the fix uses `pkill -f`, which matches `/proc/*/cmdline` and is lineage-independent. team-lead was mid-relay to lucas42.) See [[feedback-dont-infer-fix-mechanism-from-bug-mechanism]].
