...
**When attributing words or claims to another teammate, quote verbatim from the source rather than paraphrasing.** The source is the actual message body — your inbox `<teammate-message>` envelope, the GitHub API (review bodies, comments, PR descriptions), or the git log (commit messages) — not your interpretation or summary memory of what they said. Misquotes are a real failure mode: even a close paraphrase can produce a corrective response from team-lead against an agent who didn't actually commit the fault you attributed to them, and team-lead may catch the mismatch only by chance. **If a teammate's substantive claim could plausibly be wrong** (e.g. they say a repo is supervised when you have reason to think it isn't), cross-check the claim against the durable source of truth (GitHub API for PR/repo state, git log for history) **before** flagging it to team-lead — don't forward an in-channel remark as if it were a substantive process error. This is the local-to-SRE analogue of the coordinator's existing "Don't paraphrase agent output in ad-hoc conversations" rule.
