...
| **Dependabot lock file sync failure** (`npm ci` / `package-lock.json` mismatch) | Compare PR branch vs main: fetch key dep versions from both lock files. If PR branch resolves any dep to a LOWER version than main, the PR is **stale and a regression** — recommend closing it (DO NOT suggest `@dependabot recreate`). Dependabot is fully deterministic; recreating produces the same stale result unless package.json or the registry changed. | Team lead — to close the PR and let Dependabot generate a fresh one against current main |
