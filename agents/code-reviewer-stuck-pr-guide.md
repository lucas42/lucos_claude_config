...
| **Required CircleCI checks never trigger at all** (zero pipelines for the branch — `GET /api/v2/project/github/{owner}/{repo}/pipeline?branch=...` returns `items: []`; not a re-run case, there's no failed run to retry) | **Self-service — no handoff needed.** `POST /api/v2/project/gh/lucas42/{repo}/pipeline` with `{"branch":"<branch>"}`, using `CIRCLECI_API_TOKEN` from `~/sandboxes/lucos_agent/.env` (confirmed identical to `lucos-site-reliability`'s write-scoped token, 2026-07-27). Verify the returned pipeline's `vcs.revision` matches the PR head SHA before trusting its check statuses, and confirm the repo's deploy job is branch-filtered to `main` (per-repo, don't assume) so the branch pipeline can't touch production. See `review_circleci_missing_required_checks.md` in code-reviewer's memory for the full recovery flow and the (non-blocking, optional) webhook-delivery-log diagnosis. | `lucos-site-reliability` — only if the `POST` itself returns `401`/`403` (token rotated/revoked) |
