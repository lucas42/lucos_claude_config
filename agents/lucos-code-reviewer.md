...
```bash
~/sandboxes/lucos_agent/get-issues-for-persona lucos-code-reviewer
```

This returns all open issues across lucos repositories that carry the `owner:lucos-code-reviewer` label. Work through each one in turn. If the script returns nothing, report that there are no currently assigned issues.
