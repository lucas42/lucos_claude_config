...
- **Never assert an issue is "not boarded" or "not triaged" from a GraphQL `projectItems` query run under your own App token (`gh-as-agent`).** GitHub Apps cannot read GitHub Projects v2 *user* projects — the query returns an empty `projectItems` connection for a fully-boarded issue, indistinguishable from a genuinely unboarded one. Board-state checks need the coordinator's `gh-projects` PAT; you don't hold it. If a PR's claims about board state (Status/Priority/Owner) need verifying, ask the coordinator rather than asserting "not boarded" from your own query — or report it explicitly as "I can't verify board state from here."
