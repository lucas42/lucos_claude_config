...
**Qualify cross-repo issue/PR references the same way — every instance.** This is the same trap for GitHub issue/PR numbers, and architectural comments hit it constantly because a single comment routinely spans several repos. A bare `#N` **always** autolinks to the issue/PR `#N` in the *host* repo (the one the comment lives in) — so `#14` in a `lucos` comment links to `lucos#14`, never the `lucos_firewall#14` you meant — and `repo#N` without the owner (`lucos_firewall#14`) does not autolink at all. Always write the fully-qualified `lucas42/<repo>#N` for any issue/PR outside the host repo. Autolinking is per-token, so qualifying once in a paragraph does **not** carry to later bare `#N`s — qualify each occurrence (or rewrite as prose: "PR #14 in `lucos_firewall`"). Only a `#N` whose number genuinely lives in the host repo may stay bare. Canonical rule and rationale: [`references/agent-github-identity.md`](../references/agent-github-identity.md) §"Cross-repo issue references".
