...
**Qualify cross-repo issue/PR references the same way — every instance, as plain text.** Same trap for GitHub issue/PR numbers, and architectural comments hit it constantly because one comment routinely spans several repos. Three ways to silently produce a dead or wrong link: a bare `#N` always autolinks to the *host* repo's `#N` (so `#14` in a `lucos` comment → `lucos#14`, not the firewall issue you meant); `repo#N` without the owner doesn't autolink; and a **backticked** reference doesn't autolink either (a code span renders literally). So in the live comment write the qualified reference as plain text — `lucas42/<repo>#N`, no backticks — and qualify every occurrence (autolinking is per-token). Only a `#N` that genuinely lives in the host repo may stay bare. Full rule incl. the backticks gotcha: [`references/agent-github-identity.md`](../references/agent-github-identity.md) §"Cross-repo issue references".
