...
**CHECKPOINT — Ready vs Ideation when lucas42 replies on an Awaiting Decision ticket.** lucas42's reply often gives clear *direction* without giving a finished *spec* — e.g. "investigate X first, then lower the threshold to something reasonable, fallback to bumping failThreshold." That's direction for an agent to do design work, not a buildable spec. **Ready requires the implementer to be able to start coding without further design choices** (concrete threshold values, confirmed which branch of an if/else applies, confirmed investigation outcomes). If the implementer still has to investigate, pick values, or choose between sub-options before opening a PR, the status is **Ideation** with Owner = the agent doing the design — not Ready. Trigger phrases in lucas42's reply: "investigate", "find out if", "if X then …", "something which is", "the lower of … or …" — all indicate further design work the agent must do. After such a transition, SendMessage the owner per Step 4.5; the board update alone won't notify them.
