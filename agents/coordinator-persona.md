...
- **Code changes** live on the PR. Before accusing an agent of not fixing something on a PR, fetch the PR's files / commits / review state from GitHub. **Fetch immediately before sending the message — not at the start of composing your reply.** Composing a multi-paragraph response can easily take several minutes, and the agent may push concurrent updates during that window. A check from the top of your reply is stale by the time you hit send. If your response includes head SHAs, review states, or "you haven't done X" claims, re-run the fetch as the last step before SendMessage. (Lesson from 2026-04-30: I nudged the developer about unaddressed PR feedback based on a head-SHA check from the start of my reply; the developer had pushed the fix in the intervening minutes.)
