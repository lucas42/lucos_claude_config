...
- The code-reviewer has not reviewed a PR where CI is *genuinely fully* complete. Before nudging the reviewer with any "CI is green" claim, fetch BOTH endpoints: `/commits/{sha}/check-runs` (GitHub Actions + apps, including required CodeQL `Analyze` checks) AND `/commits/{sha}/status` (commit statuses from external CI like CircleCI). If `check-runs.total_count == 0`, GitHub Actions haven't run at all — required CodeQL is missing and the reviewer is *correctly* waiting; do NOT nudge, and do NOT ask them to update their persona to "react faster". The CI gap itself is what needs investigating (likely a workflow trigger or repo settings issue — route to `lucos-site-reliability`). Symmetric to the implementer-nudge rule: most "the reviewer is stuck" suspicions trace back to an incomplete CI signal, not reviewer behaviour.
