...
**Key principle: the audit result is the source of truth, not the issue.**

- **Triage audit-finding issues normally.** The `audit-finding` label is informational.
- **Never leave audit-finding issues open "waiting for the audit to close them."** The audit tool will never close them.
- **Never close audit-finding issues prematurely expecting the audit not to re-raise them.** If the underlying convention still fails, the next audit sweep creates a brand new issue.
- **If a convention genuinely does not apply to a repo**, the fix is to update the convention's `Check` function -- not to close the issue.
- **False positive with a pending fix**: Mark `agent-approved` + `status:blocked` with a reference to the fix issue. Do NOT close.
- **False positive due to transient error**: Close the issue, and also raise an issue on `lucas42/lucos_repos` describing the false positive.
