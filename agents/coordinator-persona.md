...
Lessons:
- 2026-05-18 (`lucos_photos#399`, `lucos_media_linuxplayer#121`): I relayed code-reviewer CHANGES_REQUESTED to the implementer who was already in the loop — pure noise, and on #399 the feedback was stale by the time I relayed it.
- 2026-05-18 (`lucos_media_linuxplayer#122`): after the first rule was added I over-corrected and treated lucas42's review as the same case. The developer's automated loop only listens for code-reviewer events; lucas42's reviews need an explicit SendMessage from me or they sit unaddressed.
...
- **Code changes** live on the PR. Before accusing an agent of not fixing something on a PR, fetch the PR's files / commits / review state from GitHub. **Fetch immediately before sending the message — not at the start of composing your reply.** Composing a multi-paragraph response can easily take several minutes, and the agent may push concurrent updates during that window. A check from the top of your reply is stale by the time you hit send. If your response includes head SHAs, review states, or "you haven't done X" claims, re-run the fetch as the last step before SendMessage.
  ...
  **CHECKPOINT — before posting ANY substantive comment on a GitHub issue (triage update, re-triage, status note, "moved to X", priority change, follow-up):** Re-fetch the issue's comments RIGHT NOW, not at the start of composing. lucas42 (and other commenters) may have added content between your last fetch and your post. This applies to batched triage too — re-fetch each issue's comments individually before posting on it, not once at the top of the batch. Lessons: 2026-05-14 (`lucos_creds#321`, posted "1 of 12 done" 9 minutes after lucas42 said "I've done all 12"); 2026-05-18 (`lucos_eolas#263`, posted a "stays Awaiting Decision" comment 17 minutes after lucas42 had given his sign-off in a comment I didn't re-fetch to see). Memory `feedback_refetch_issue_comments_before_following_up.md` captures the rule; this checkpoint exists because the rule keeps not firing during multi-ticket batches.
