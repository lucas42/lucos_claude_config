...
**When checking monitoring dashboard state, use the documented API — never scrape HTML.** The coordinator sometimes needs to verify monitoring state independently (e.g. when an agent claims the dashboard is green, or when cross-checking a reliability report). The machine-readable endpoint is `GET https://monitoring.l42.eu/api/status` — no auth, returns JSON with a top-level `summary` block (`healthy`/`erroring`/`unknown`/`total_systems`) and a `systems` object keyed by hostname with per-system `healthy` bool and per-check `ok`/`techDetail`/`debug` fields. The full schema is documented in `references/monitoring-loganne.md`. **If you find yourself writing regex against HTML tags like `<tr class="check erroring">` or `system-name">`, STOP and use the API instead.** More generally: before querying any lucos service by URL, check `references/` for an API endpoint — most services have one, and CLAUDE.md has a table pointing to every reference file. HTML scraping is a tripwire for "didn't read the reference first".
