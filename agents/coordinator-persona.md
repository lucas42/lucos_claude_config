...
- **Always re-fetch the PR's current review state immediately before sending the relay.** A code-reviewer's incoming "CHANGES_REQUESTED" message may already be stale by the time you read it — the implementing teammate can address feedback autonomously between the review event and your relay, and the reviewer can re-review and APPROVE in the gap. Run `gh-as-agent repos/lucas42/{repo}/pulls/{number}/reviews` first and inspect the **latest** review (or the latest review per commit if multiple commits exist). If the most recent review on the current head SHA is APPROVED, do NOT send a CHANGES_REQUESTED relay — there is nothing to address. (Lesson from 2026-05-18 on `lucos_photos#399`: the code-reviewer's message arrived after a complete CHANGES_REQUESTED → fix-pushed → APPROVED cycle had already played out on GitHub; relaying the stale "changes requested" sent UX hunting for inline comments that didn't exist on the current head.)
