...
**Never write `#N` for an alert number** — not in the issue you file, the fix PR's title/body/commits, or the completion-manifest SendMessage to whoever requested the run. This is the step where alert numbers actually get typed, so the check happens here, not just once in the persona file. Qualifying it as `owner/repo#N` does **not** make it safe — that fixes cross-repo ambiguity, not the alert-vs-issue/PR ambiguity, and a qualified alert number silently cross-links whatever real issue/PR holds that number instead (caught on lucas42/tfluke#528, 2026-09-13 — the alert-number habit showed up in both the PR body and the manifest). Before sending or posting anything that mentions an alert, grep your own draft for the alert's number and swap it for the CVE/GHSA id (or the full alert URL if neither exists) — see [`references/github-config.md`](../references/github-config.md) § "GitHub comment conventions" for the canonical rule.
...
- **Before investigating further, verify the advisory's affected-version range actually covers the installed version.** Fetch the alert details (`repos/lucas42/{repo}/dependabot/alerts/{number}`) and check `security_vulnerability.vulnerable_version_range` and `security_vulnerability.first_patched_version` against the version in the lockfile. Watch in particular for cross-major-version raw-semver comparisons: if the patched version and the affected range both live in a different major line from what's installed (e.g. advisory says `≤ 4.1.1`, patched `4.2.0`, but installed version is `3.14.2`), the alert is likely a false positive — Dependabot has flagged a package because `3.x < 4.1.1` numerically, not because the 3.x branch is genuinely in scope. If no fix exists for the installed major line (no 3.x patched version listed), and there is no public discussion of the CVE affecting the installed major line, treat the alert as out-of-scope and skip it.
