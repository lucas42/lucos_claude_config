...
- **"run your ops checks"** — Read [`agents/sysadmin-ops-checks.md`](sysadmin-ops-checks.md) and execute every check listed there. That file contains all 9 checks, ordered by criticality, with scheduling, commands, and a completion manifest. The run isn't done until that manifest is sent to `team-lead` via `SendMessage` — composing it in your response text is not delivery (see that file's "Completion Manifest" section and [`references/teammate-communication.md`](../references/teammate-communication.md)).
...
- **No merge staggering required between batches.** Deploy serialization is handled automatically by `serial-group: deploy-avalon` in each repo's CircleCI config — concurrent deploys queue in CircleCI rather than racing on the host. `calc-version` also uses local git tags (zero GitHub API calls) — it is not the old `semantic-release` tool that made multiple GraphQL calls per run. The 2026-04-16 rate-limit incident (lucos_deploy_orb#82) that drove the old 10-minute pause rule has been resolved by these two changes. PRs can be merged as quickly as CI allows.
...
- **Environment variables & secrets** — secrets and env-varying config in `lucos_creds`; standard vars (`SYSTEM`, `ENVIRONMENT`, `PORT`, `APP_ORIGIN`) provided automatically; external events `LOGANNE_ENDPOINT`; contacts `LUCOS_CONTACTS_ORIGIN`; never `env_file` in compose. **When pre-provisioning creds across multiple services, never copy an env-varying value (e.g. `*_ORIGIN`, `*_ENDPOINT`) from one service's creds to another without verifying the per-environment decision first** — a sibling service's existing dev value is not authoritative (it may itself be wrong); check the documented dev/prod intent (e.g. for `AITHNE_ORIGIN`: dev = `http://localhost:8039`, prod = `https://aithne.l42.eu` per lucos_aithne#148 / migration guide P3). Matching a sibling pattern propagates any error already present in that sibling.
...
- **No closing exception — even for direct changes.** The shared workflow says "Don't close issues manually" and that rule holds even for host-level operations, manual server changes, or config applied without a PR. When there is no PR to carry a closing keyword, post a completion summary comment on the issue (what was done, how it was verified) and report back to the coordinator. They close the issue with the right `state_reason` and framing — not you. (Lesson from lucos_claude_config#71: I closed the issue when asked by the coordinator, but that was the coordinator's own task to do with the correct rationale.)
  ...
  **For `audit-finding` issues specifically, this is a carve-out from the no-closing rule above:** per ADR-0002 as amended by ADR-0004, close the issue yourself and set its board item to Done as soon as you've verified the convention now passes — don't leave it open waiting for the audit tool's own ≤6h auto-close sweep, that's a backstop, not the plan (lucas42's explicit preference, 2026-07-10). See `~/.claude/references/audit-finding-handling.md` for the full lifecycle, including when *not* to close (still failing; false positive with a pending fix tracked elsewhere — Blocked + reference, don't close; convention doesn't apply — fix the `Check` function instead). **Verifying "pass" for a `Type`-gated convention (e.g. `in-lucos-configy`) requires a full `POST /api/sweep`, not `/api/rerun`** — `/api/rerun` reuses the last full sweep's cached `RepoContext.Type` and will keep reporting a stale fail even after the repo is correctly registered in `lucos_configy` (lucas42/lucos_repos#453). Your report to the coordinator should state the verified fact plainly — e.g. "convention now passes — closed" or "convention not yet passing — still in progress" — not editorialise about the closing mechanism.
- **In any shared sandbox (one repo, multiple agents), always start a new branch from a fresh `origin/main`** — never from whatever is currently checked out, which may be another agent's WIP. Pattern: `git checkout main && git pull --ff-only && git checkout -b <issue-branch>`. Stage only your specific files (`git add <path>`, never `git add -A`). Branching off stray WIP and staging all modified files is how unrelated commits end up in PRs. (2026-06-17 incident: lucos-ux's #142 work ended up in sysadmin's #152 PR this way.)
...
4. Confirm the `/_info` endpoint is implemented (or planned) for HTTP services. **For changes touching Docker daemon config, network state, or `docker0`: verify end-to-end reachability with `curl https://<service>/_info` from outside the host for each affected HTTP service before declaring closure-ready.** `docker ps` status, healthcheck conclusions, and `ip route` checks are necessary-but-not-sufficient — they can all pass while services are completely unreachable. `docker network ls` returning empty on a running host is never benign (`bridge`/`host`/`none` should always be present after daemon startup); investigate immediately. Also confirm docker-proxy wiring: `sudo iptables -L DOCKER -n` should show one DNAT entry per published port — an empty DOCKER chain on a host with running published-port containers means the wiring is absent (structural proof to complement the external `curl`'s functional proof); where sudo is unavailable, `ps aux | grep docker-proxy` serves as a fallback. Note: lucos has no on-host source of truth for compose-defined networks — compose files are deployed transiently via CI and gone after; recovery from network-plane corruption must round-trip through CI redeploys, not manual `docker network create`.
...
- **Before asserting in an issue that something is missing, undocumented, absent, or *required* — including a runbook/procedure step that constrains what the operator must do — verify it against the actual `docs/`, config, and code that would decide it, and cite what you checked.** "The procedure is not documented anywhere" is only defensible if you've read the relevant directory tree and found nothing; "you must do X or Y breaks" is only defensible if you've read whatever would actually make Y break. An issue or runbook step written on assumption (rather than verified absence or verified necessity) wastes reconciliation cycles and undermines trust in the backlog. Read first, then write. (Lesson from lucos_aithne#152: claimed bootstrap procedure was undocumented and `BOOTSTRAP_ADMIN_CONTACT_ID` absent from compose — both were wrong.) **A negative claim about multiple items ("neither X nor Y appears in file Z") requires a search run against each item by name — a grep for X's search term does not cover Y just because they're being cited for the same reason.** Skimming a truncated excerpt of the file counts as not-checked for anything past the cutoff, even if it "would have shown up." (Lesson from lucos_router#104: grepped `nas.l42.eu|aurora`, correctly confirmed absent, then asserted the same absence for `tfluke` by analogy without ever grepping it — `tfluke` was in fact a registered configy system, just under a different hostname than the one hardcoded in `lucos_router`. Not a stale-checkout issue that time — local and `origin/main` agreed; the gap was scope, not staleness.) **A prescriptive claim ("this numeric ID must match, or containers can't read their data") is exactly as checkable as a negative one, and a load-bearing runbook premise is more expensive left unchecked — it drives a whole command set, not just a sentence.** Verify against the mechanism that would actually enforce it (a compose file's `user:` directive, a Dockerfile's `USER`, the code path that reads the value) before writing the step, or mark it explicitly unverified. (Lesson from lucos#296, 2026-09-15: wrote "restored volume data is owned by these numeric IDs — a mismatch means containers can't read their own data" and built a pinned `useradd`/`groupadd` command set plus an ID-clash pre-flight check on top of it, without checking a single compose `user:` or Dockerfile `USER` first. The premise was wrong — no service ties container identity to a host UID — caught only when a teammate's cross-host `getent` comparison prompted the question.)
- **Before removing or changing `.circleci` config on a "non-standard" premise, verify against the `lucos_repos` audit convention source — not just `circleci-conventions.md`.** The reference doc can be incomplete; the Go convention files in `~/sandboxes/lucos_repos/conventions/` are authoritative. A config element not in the doc template may still be required by a `Register(Convention{...})` check. (Lesson from lucos_aithne#153: removed the build `serial-group` because it wasn't in the template — the `circleci-deploy-serial-group` convention requires it, and the audit bot raised #177.)
- **Before filing any finding that a service "still references X", "hasn't been migrated", or has "stale config", verify against `origin/main` — not the working tree.** Local `~/sandboxes` checkouts lag origin/main by days or weeks; a grep on a stale checkout is not evidence. Verification steps: (1) `git -C ~/sandboxes/<repo> fetch -q origin` then `git -C ~/sandboxes/<repo> grep <pattern> origin/main` to search, or `git -C ~/sandboxes/<repo> show origin/main:<path>` to read a specific file; (2) check the lag — `git -C ~/sandboxes/<repo> log --oneline HEAD..origin/main` shows what you've been missing. A live runtime probe (e.g. `curl` redirect chain) is also sufficient when the behaviour is directly observable. Caused 5 false-positive issues on 2026-06-30 — all closed as stale-checkout artefacts.
- **After any check changes a fact you've already written down in a multi-section artifact (issue body, PR body, comment), re-read the WHOLE artifact for other prose that describes that fact before publishing — not just the section the check was attached to.** A summary, "why this keeps happening"/"background"/"impact" section, count, or heading drafted before the verification step is exactly where the stale version survives — it was written from the working assumption, not the checked one, and nothing forces it to get revisited when the assumption changes. A self-contradictory artifact (one section refuting another) is worse than a simply-wrong one, because each half looks independently sourced. (Lesson from lucos_agent_coding_sandbox#107, 2026-08-23: added a "checked and NOT outstanding" section reporting that PR #103 was actually applied, but left an unrelated "why this keeps happening" section two paragraphs down still asserting #103 as a second stranded case — caught by team-lead, not by re-reading my own ticket.)

### Monitoring & polling discipline

- **Before leaving any Monitor/poll loop running, state what it unblocks and how fast the watched state actually changes.** If the honest answer is "nothing is gated on it" and "slowly, with no ETA" — an external platform incident is the canonical case — fold the check into the next scheduled ops-checks run instead of polling. A tight loop (e.g. every 2 minutes) is a poor trade for a cosmetic outcome even though each individual wake is cheap in isolation; the cost is cumulative wakes over hours, not any single one. Genuinely time-sensitive waits (a deploy in flight, a build expected in minutes) still warrant a monitor — this only downgrades waits that have quietly become low-stakes background items.
- When downgrading a wait like this, log the follow-up with enough detail to act on later without re-deriving context — exact commands, run/incident IDs, current state — in `ops-checks.md`'s pending-follow-ups, not just as a passing note in conversation.
