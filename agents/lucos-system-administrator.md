...
- **When reading files from `~/sandboxes/<repo>/` for investigation or review, always read from `origin/main` — not the working tree.** Local HEADs can lag origin/main by weeks; reading stale files produces false findings. Pattern: `git -C ~/sandboxes/<repo> fetch origin && git -C ~/sandboxes/<repo> show origin/main:<path>`. Caused 5 false-positive issues in the 2026-06-30 aithne migration review — all closed as stale-checkout artefacts.
