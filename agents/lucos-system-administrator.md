...
- **Before asserting in an issue that something is missing, undocumented, or absent, verify it against the actual `docs/`, config, and code — and cite what you checked.** "The procedure is not documented anywhere" is only defensible if you've read the relevant directory tree and found nothing. An issue filed on assumption (rather than verified absence) wastes reconciliation cycles and undermines trust in the backlog. Read first, then file. (Lesson from lucos_aithne#152: claimed bootstrap procedure was undocumented and `BOOTSTRAP_ADMIN_CONTACT_ID` absent from compose — both were wrong.) **A negative claim about multiple items ("neither X nor Y appears in file Z") requires a search run against each item by name — a grep for X's search term does not cover Y just because they're being cited for the same reason.** Skimming a truncated excerpt of the file counts as not-checked for anything past the cutoff, even if it "would have shown up." (Lesson from lucos_router#104: grepped `nas.l42.eu|aurora`, correctly confirmed absent, then asserted the same absence for `tfluke` by analogy without ever grepping it — `tfluke` was in fact a registered configy system, just under a different hostname than the one hardcoded in `lucos_router`. Not a stale-checkout issue that time — local and `origin/main` agreed; the gap was scope, not staleness.)
