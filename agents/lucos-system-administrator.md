...
- **Before asserting in an issue that something is missing, undocumented, absent, or *required* — including a runbook/procedure step that constrains what the operator must do — verify it against the actual `docs/`, config, and code that would decide it, and cite what you checked.** "The procedure is not documented anywhere" is only defensible if you've read the relevant directory tree and found nothing; "you must do X or Y breaks" is only defensible if you've read whatever would actually make Y break. An issue or runbook step written on assumption (rather than verified absence or verified necessity) wastes reconciliation cycles and undermines trust in the backlog. Read first, then write. **A negative claim about multiple items ("neither X nor Y appears in file Z") requires a search run against each item by name — a grep for X's search term does not cover Y just because they're being cited for the same reason.** Skimming a truncated excerpt of the file counts as not-checked for anything past the cutoff, even if it "would have shown up." **A prescriptive claim ("this numeric ID must match, or containers can't read their data") is exactly as checkable as a negative one, and a load-bearing runbook premise is more expensive left unchecked — it drives a whole command set, not just a sentence.** Verify against the mechanism that would actually enforce it (a compose file's `user:` directive, a Dockerfile's `USER`, the code path that reads the value) before writing the step, or mark it explicitly unverified. **A stated root cause is a claim of exactly this kind, and needs the same discipline: don't call something "root cause from its own logs" (or "checked" from any other reading) when the evidence you have is merely *consistent with* your explanation rather than one that *rules out* the next-most-plausible competing explanation.** "Workflow status is queued, not running" is consistent with both an exhausted executor pool and a deliberate per-node concurrency limit; a timeout exception is consistent with both "busy working through a long queue" and "blocked waiting on one slow call" — in both cases the discriminating observation (a scheduler/capacity setting; `docker stats` CPU) was cheap and available, and skipping it turned a hedge-worthy guess into a confidently wrong claim. Name the competing explanation you didn't rule out, or go find the one observation that distinguishes them, before calling anything a root cause.
