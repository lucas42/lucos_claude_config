...
- **REQUEST SPECIALIST REVIEW** — domain expertise needed:
  ...
  **Mandatory security sign-off — always-review repos (repo-scoped, always):** *every* PR on a repository in the list below requires `lucos-security` review, regardless of what the change touches — any change to these is security-relevant by definition. On any PR in one of these repos you **must** request `lucos-security` review (follow "Request specialist review" below) and you **must not `APPROVE` until `lucos-security` has posted a go-ahead on the PR itself** (a GitHub review/comment, not a SendMessage — see the GitHub-artifact rule below). This is in addition to the change-nature triggers above.
    ...
    **Exception — `lucos-security`-authored PRs (self-approval is structurally impossible).** When `lucos-security` is the *author* of the PR, GitHub will not accept a formal approving review from them on their own PR, so the normal go-ahead artifact is unavailable. In that case an explicit, unambiguous **approving comment** from `lucos-security` on the PR — a clear security sign-off / "good to merge", not merely a neutral or descriptive remark — satisfies the security-sign-off requirement: treat it as the go-ahead and `APPROVE`. This applies *only* when security is the PR author and the formal approval is structurally blocked; for every other author, a formal `lucos-security` review/approval is still required. (Confirmed by lucas42, 2026-06-12, resolving the lucos_aithne#98 self-authored-fix deadlock.)

  **Self-authored always-review PRs:** When `lucos-security` is the *author* of a PR on an always-review repo, GitHub blocks them from submitting a formal approving review on their own PR. In this case, an **explicit approving comment** from `lucos-security` on the PR counts as the required security sign-off — the bar is a clear, unambiguous statement (e.g. "security sign-off", "good to merge from a security perspective"), not just any comment. Treat it as equivalent to a formal GitHub APPROVE and proceed to issue your own APPROVE. Do not deadlock waiting for a formal review that GitHub prevents. (Rule confirmed by lucas42 2026-06-12, resolving PR #98 deadlock.)
...
2. **Check auto-merge.** Wait ~30 seconds after approval, then re-fetch the PR — fetch `{state, merged_at, auto_merge}`.
   ...
   - **`auto_merge` non-null:** queued auto-merge enabled, will merge when CI passes. Report as "auto-merge enabled".
