...
**`mergeable_state: blocked` with everything green on the REST endpoints above is NOT proof the block is stale.** The REST `/check-runs` endpoint shows what ran on the SHA — it is not the surface branch protection actually gates on. A required check can have `conclusion: success` there and still be **absent from the PR's check-suite rollup** (typically after a dropped `pull_request` webhook where the check was later re-triggered via `workflow_dispatch`, which creates a check-run but never associates it with the PR) — branch protection evaluates the rollup, not raw check-runs, so it keeps blocking. Before calling a persistent `blocked` state "probably a stale recompute," query the GraphQL rollup directly — see `code-reviewer-stuck-pr-guide.md` Criterion 8 for the query and the close+reopen fix. Two people independently agreeing a block is stale is not verification; only the rollup query is. (Confirmed failure: lucos_worlds#72, 2026-08-06 — both `lucos-site-reliability` and `lucos-code-reviewer` read REST check-runs as green and agreed the block was stale; team-lead caught it, GraphQL rollup showed `Analyze (python)` genuinely absent from the 8-context rollup.)

**The rollup's own top-level `state` field is not trustworthy either — check the named `contexts` list, not the summary.** `statusCheckRollup.state` can itself read `SUCCESS` while a required context is missing from `contexts` entirely (confirmed on lucos_worlds#73 the same night — 7 green CircleCI contexts, aggregate `state: SUCCESS`, zero CodeQL contexts of any kind). Always confirm the specific required-check *name* appears in `contexts` — never infer "satisfied" from the aggregate `state` alone.
...
2. **Check auto-merge.** Wait ~30 seconds after approval, then re-fetch the PR — fetch `{state, merged_at, auto_merge}`.
   - **`state: closed` / `merged_at` non-null:** PR already merged (directly, without queuing). Report as "merged". Stop here — do not interpret `auto_merge`, and do not add *any* cause/attribution commentary (e.g. "merged because the repo is unsupervised", "on bot approval") without first doing the check in the "characterise what triggered it" bullet below — a bare "merged" report is always safe; a guessed cause is not, even as a passing parenthetical.
   ...
   - **If the PR has merged and you want to say *anything* about what triggered it** — including a throwaway parenthetical like "(unsupervised, so it merged on approval)" — that is characterising the merge, and the rule below is not optional for it: fetch the reviews list — `repos/lucas42/{repo}/pulls/{n}/reviews --jq '.[] | {state, submitted_at, user: .user.login}'` — and compare approver timestamps against `merged_at`. The merge trigger is the approval that immediately precedes `merged_at`, not the earliest approval in the list. A small gap (seconds to minutes) between your own approval and the merge does NOT imply your approval triggered it — lucas42 may have approved in between, even on a repo you already confirmed supervised earlier in the same session. Never assert a supervision gap, or write "unsupervised"/"on bot approval" anywhere in a report, without this check run fresh for *this* merge. (Confirmed failure: lucos_backups#407, 2026-08-28 — reviewer had already run `check-unsupervised lucos_backups` earlier in the same session and gotten "supervised", then reported the same PR's merge as caused by "lucos_backups is unsupervised" without re-checking either that answer or the reviews list; lucas42's approval, not the bot's, had triggered the merge 25s later.)
