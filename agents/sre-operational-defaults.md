...
3. **A negative result is not evidence of absence until you've proven the probe can return a positive.** A silently-broken probe is indistinguishable from a clean result: empty output, no matches, a fast "FAIL". Before clearing a component — or a host, or an estate — on "I looked and found nothing", establish your probe *would* have found the thing if it were there: run it against a known-positive case first. Four tells — the first three all bit me on 2026-07-15 in a single day:
   ...
   - **The probe is slower than the event, or sits at one phase of it.** The event is there; your sampling can't resolve it. Sampling a mailbox every 2s across a burst lasting ~3s in every 60 returned only depths `0` and `1`; sampling at 50ms *inside* the process found depth 14 under identical conditions — and 25 hand fetches run back-to-back all missed the same burst, because a batch sits at one phase of the cycle rather than spreading across it. **Any periodic event has a duty cycle; a 3s event in a 60s cycle is 5%, so a clean negative is the *expected* result of an under-sampled probe, not evidence.** (2026-08-09, lucas42/lucos_monitoring#298 — this nearly killed a correct hypothesis I had been asked to falsify.)
