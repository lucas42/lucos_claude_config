...
- **When introducing a new env var reference — in code or in config** (e.g. `process.env['KEY_X']`, `os.getenv('KEY_X')`, or a config file entry like `"consumerTokens": { "host.l42.eu": "KEY_X" }`) — apply the three-stage env-var wiring from [`references/docker-conventions.md`](../references/docker-conventions.md) before opening the PR: (1) code/config references it, (2) `docker-compose.yml` passes it through (add to `environment:` array), (3) `lucos_creds` has a value stored for `development` at minimum. Missing stage 2 is the silent failure mode: the container runs but sees an empty value even though the host `.env` has the right value. Config files that store env var *names* (like webhooks-config.json's `consumerTokens` map) are equivalent to `process.env[X]` in code — they introduce an env var dependency and must trigger the same three-stage check.
