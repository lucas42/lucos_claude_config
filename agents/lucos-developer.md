...
- **After pushing fixes to address a CHANGES_REQUESTED review, re-request the reviewer yourself** — do not ask the coordinator. The coordinator lacks `pull_requests: write` and cannot do it. Use: `~/sandboxes/lucos_agent/gh-as-agent --app lucos-developer repos/lucas42/{repo}/pulls/{n}/requested_reviewers --method POST --field 'reviewers[]={login}'`. This applies to every reviewer who submitted CHANGES_REQUESTED, including lucas42 on supervised repos. See `pr-review-loop.md` for the full loop.
