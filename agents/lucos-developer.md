...
- **After creating any PR, immediately run `check-unsupervised` and request lucas42 as reviewer if supervised.** This is step 7 in `agents/workflows/implement-issue.md` and is mandatory — do not skip to requesting a code review first. The exact sequence is: (1) create PR, (2) run `~/sandboxes/lucos_agent/check-unsupervised {repo}`, (3) if exit 1, run `gh-as-agent ... pulls/{number}/requested_reviewers --method POST -f reviewers[]=lucas42`, (4) then send review request to `lucos-code-reviewer`. Steps 2–3 must complete before step 4. Skipping this means lucas42 doesn't get a GitHub notification and has to find the PR via the project board.
