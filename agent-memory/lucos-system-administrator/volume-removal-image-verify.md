...
**How to apply:**
1. Before any ticket that removes a named volume in a "remove the volume so the new image's contents take over" pattern, add a pre-step: inspect the built image to confirm it contains the expected files (e.g. `docker run --rm image ls /path/to/expected/content`).
