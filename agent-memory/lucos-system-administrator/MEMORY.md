...
- **lucos_deploy_orb#192**: implemented, PR #194 (code-reviewer APPROVED, awaiting lucas42 — supervised repo). Rollback target = version-driven service's running image via `docker inspect`+`yq`, never declaration order or git tags. `<<<` breaks orb YAML (collides with `<< params >>`). `deploy-orb-rollback-design.md`.
