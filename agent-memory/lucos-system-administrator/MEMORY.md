...
- **lucos_worlds/BookStack deploy (2026-07-07)**: MariaDB password-drift-after-init gotcha (wipe volume, don't just reset creds); Laravel `APP_KEY` needs literal `base64:` prefix; **BookStack's OIDC client hardcodes RSA/RS256-only — incompatible with lucos_aithne's ES256-only signing, no config fix exists**; branch protection must exist before the first PR, not after. `lucos-worlds-bookstack-deploy.md`.
