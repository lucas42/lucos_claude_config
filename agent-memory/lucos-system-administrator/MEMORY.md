...
- **lucos_deploy_orb#195** (Needs Analysis, mine, Medium): BuildKit default manifest media type Docker v2→OCI v1, unpinned `buildx-stable-1` tag, estate-wide (confirmed live on `lucos_backups:latest`), not a #186 recurrence (still single manifest, not index). `buildkit-oci-manifest-drift.md`.
