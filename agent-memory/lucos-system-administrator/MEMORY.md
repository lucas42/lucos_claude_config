...
- **PR merges never reach `~/.claude`'s working tree** — #127's mixed reset only converges HEAD/index, never files; manually materialize + verify-by-executing after every merge until lucos_claude_config#134 ships a fix. `pr-merge-never-reaches-working-tree.md`.
