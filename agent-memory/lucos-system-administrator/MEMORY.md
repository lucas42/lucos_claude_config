...
- **lucos_docker_mirror#88**: BLOCKED 2026-08-31 — recorded decision (`proxy.ttl: 0`) is a no-op on the pinned `registry:2.8.3` image (no `TTL` field in that version's config struct; added later in v3.x); non-strict YAML silently drops unknown keys instead of erroring. Flagged on issue + to team-lead before implementing. `lucos-docker-mirror-88-registry-ttl-nonexistent.md`.
