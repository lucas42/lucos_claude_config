...
- **lucos_deploy_orb#192** (deploy rollback design): rollback target = running container's image via `docker inspect`, never git tags — "second-newest tag" picks another broken version in the actual incident data. Fix lives in the orb (shared across ~55 repos via floating `@0` pin), not per-service compose. `deploy-orb-rollback-design.md`.
