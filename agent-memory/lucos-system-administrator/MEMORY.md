...
- **Self-check before writing a code/config comment**: "would this survive as one line?" — rationale belongs in the commit message, not in-code (lucas42 correction, lucos_deploy_orb#201). `feedback_concise_code_comments_self_check.md`.
...
- **`commit-claude-main --delete`** (lucos_agent#73) removes a file from `~/.claude` origin/main without disk presence. `commit-agent-memory.sh` now strips `*.tmp.<pid>.<hash>` before staging (temp-file leak fix). `commit-claude-main-delete-support.md`.
- **lucos#278**: both network recreations (xwing dns_secondary, avalon monitoring) succeeded 2026-08-09 but sat unreported 9 days — always post a result comment even when nothing broke. Step 2 (6× `NODE_OPTIONS` PRs) never started. `lucos-278-network-recreation-reporting-gap.md`.
...
- **lucos_agent_coding_sandbox**: `pi-hosts`/`avalon-host` scripts have no deploy path — merge ≠ applied live (no sudo on xwing/salvare/avalon). Stranded PR #103 and #106; tracking issue #107 filed 2026-08-23. `lucos-agent-coding-sandbox-no-deploy-path.md`.
- **lucos_agent#78** (Needs Analysis, mine, Medium): should a partial-coverage test report success at all? Split from creds-in-CI question (gated separately). 4th "vacuous green" instance in one session 2026-08-23. `lucos-agent-78-vacuous-green.md`.
- **lucos_deploy_orb#192**: REJECTED by lucas42 2026-08-28 (cost/benefit, not quality — "too much complexity for a rare occurrence"), PR #194 closed unmerged. Don't reopen/propose slimmer variant unless asked. #193 (version-resolution fix) survives, Ready/High/mine, undispatched. `deploy-orb-rollback-design.md`.
- **lucos_deploy_orb#195**: RESOLVED 2026-09-01 (PR #201) — assert "not an index" not exact media-type string. **#196** (filed, Awaiting Decision, lucas42, Low): pin-vs-drift decision on BuildKit's default builder image — Dependabot can't track it (raw string in CircleCI command, not Dockerfile/compose). `buildkit-oci-manifest-drift.md`.
- **lucos_agent#72**: RESOLVED 2026-09-01 (PR #84/#86) — `prune-images` never ran 3mo: `<< pipeline.trigger.type >>` uses "schedule", REST API uses "scheduled_pipeline" — different vocabularies for the same trigger. Confirmed live via temp diagnostic job before shipping. xwing 48.94GB→0B reclaimed. Avalon CI-watchdog issue → `lucos_deploy_orb`#203. `circleci-trigger-type-vocabulary-mismatch.md`.
- **lucos_agent#79**: RESOLVED 2026-08-31 (PR #83) — `get-issues-for-triage` criterion 3 now excludes `Status=Ready` (falls through to criterion 5); verified live before/after, one example cross-checked via direct GraphQL. `lucos-agent-79-triage-ready-exclusion.md`.
- **lucos_claude_config#147**: RESOLVED 2026-08-31 (PR #156) — `return-to-main.sh`'s `CLAUDE_DIR`/`LOGANNE_EVENT_SCRIPT` now parameterised; new fixture-based test suite (11 checks), verified to actually catch regressions via sabotage. `lucos-claude-config-147-return-to-main-fixture.md`.
- **lucos_agent#76**: RESOLVED 2026-08-31 (PR #82) — `verify-teammate-quote`'s `PROJECTS_DIR` now overridable via env var; test uses a synthetic fixture, no longer hardcodes a real session UUID. Wired into CI. `lucos-agent-76-fixture-based-quote-test.md`.
- **lucos_docker_mirror#88**: SHIPPED 2026-08-31 (PR #91) — original `proxy.ttl:0` decision was a no-op on pinned 2.8.3 (no `TTL` field; caught before implementing). Sized + rehearsed `registry:2`→`registry:3` bump live with Docker (config path moves to `/etc/distribution/config.yml`, load-bearing), shipped it, verified post-deploy against the real 52.8GB cache. `lucos-docker-mirror-88-registry-ttl-nonexistent.md`.
