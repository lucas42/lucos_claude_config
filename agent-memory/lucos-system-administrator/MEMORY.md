...
- **lucos_deploy_orb#195** (Ready, mine, Medium): assert "not an index" not exact media-type string. **#196** (filed, Awaiting Decision, lucas42, Low): pin-vs-drift decision on BuildKit's default builder image — Dependabot can't track it (raw string in CircleCI command, not Dockerfile/compose). `buildkit-oci-manifest-drift.md`.
