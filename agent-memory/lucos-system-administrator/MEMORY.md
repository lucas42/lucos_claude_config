...
- **Audit-finding issues**: never auto-closed by the audit tool — coordinator closes once the convention passes. `feedback_audit_finding_no_autoclose.md`.
...
- **Pre-scaffolding repo → configy safely, if type is already settled**: bare `null` entry (no domain/http_port/hosts) is invisible to monitoring but satisfies `in-lucos-configy`; only safe when type isn't ambiguous (contrast `lucas_architecture_models`, left out). `configy-undeployed-system-entry-pattern.md`.
