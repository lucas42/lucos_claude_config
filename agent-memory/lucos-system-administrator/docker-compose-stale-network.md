...
**Also applies to:** `lucos_monitoring_default`, `lucos_time_default` — both have `enable_ipv6: true` declared in compose but live networks are IPv4-only (never recreated). ~~Not currently causing issues since neither needs IPv6 to reach its targets.~~ **Superseded 2026-08-08:** that call was wrong. Node's Happy Eyeballs (`autoSelectFamilyAttemptTimeout`=500ms, ahead of Linux's ~1s SYN retransmit) amplifies packet loss into hard `fetch failed` errors specifically when the caller has **no IPv6 egress at all** — a different condition than "does this specific target need IPv6", which is the only thing I checked in June. Both networks' missing IPv6 became a real amplifier during the 2026-08-08 home-link packet-loss incident (`docs/incidents/2026-08-08-home-link-packet-loss.md`, lucas42/lucos#278/#279). **Lesson: "not needed for its declared targets" is not the same question as "does this container have any IPv6 egress" — check the latter, not just the former, when assessing whether a missing IPv6 network is actually benign.**

**Confirmed on:** avalon (2026-06-08) during lucos_backups#307. Drift also independently reconstructed by lucos-site-reliability on 2026-08-08 (lucas42/lucos#279) without knowledge of this June finding — the mechanism matches exactly, corroborating both.
