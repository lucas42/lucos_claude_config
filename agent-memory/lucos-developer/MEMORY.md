...
- **`php:*-apache` images do NOT include `wget` or `curl`** — they're `debian:bookworm-slim` based. Install explicitly: `RUN apt-get update && apt-get install -y --no-install-recommends curl && rm -rf /var/lib/apt/lists/*`. Prefer `curl -sf` over `wget -qO-` for healthchecks — `-f` treats HTTP errors as failures, which wget doesn't do by default.
