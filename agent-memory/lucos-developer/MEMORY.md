...
**Python/PyJWT pattern (lucos_photos#455, PR #465)**: `PyJWKClient` (>=2.13.0) has NO `jwk_set_data` attribute — that's from an older PyJWT version. Its real cache is `self.jwk_set_cache` (`JWKSetCache`), and critically `fetch_data()` calls `self.jwk_set_cache.put(jwk_set)` in a `finally` block even on failure (`jwk_set=None`), which **wipes the cache** rather than preserving it. A subclass overriding `get_jwk_set` must maintain its own independent snapshot (`self._last_known_good`, set only on success) — reusing `self.jwk_set_data` or the parent's own cache both fail silently/loudly. lucos_photos already had a `_ResilientJWKSClient` that looked correct but referenced the nonexistent `jwk_set_data` attribute — undetected because its tests mocked `get_signing_key_from_jwt` directly (skipping `get_jwk_set` entirely) rather than exercising the real class. **Check any other Python/PyJWT consumer for the same `jwk_set_data` copy-paste bug before assuming "PyJWKClient-based = fine."**
