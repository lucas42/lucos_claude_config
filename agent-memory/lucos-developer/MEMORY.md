...
Split from lucos_aithne#241 into per-consumer issues. Pattern: wrap jose's `createRemoteJWKSet` in a `createServeStaleJWKS` helper that snapshots `.jwks()` after each success and falls back to `createLocalJWKSet(snapshot)` on an infra error. The issue's own suggested snippet (`lastKnownGoodKeys = remoteJWKS`) is a no-op — don't carry it forward uncritically. **`isJWKSInfraError` must classify by exact code (`ERR_JWKS_TIMEOUT`/`ECONNREFUSED`/`ENOTFOUND`), NOT `startsWith('ERR_JWKS_')`** — the broad prefix match also catches `ERR_JWKS_NO_MATCHING_KEY`, which jose only throws after its own internal reload-and-retry already failed to find the kid in the freshest key set; treating it as infra logs a false "aithne unreachable" warning on routine token rejections and triggers a useless fallback (caught by code-reviewer on both #552 and #564 — get this right on the next consumer, e.g. lucos_arachne/lucos_notes, first time). Done: lucos_media_seinn#543 (PR #552), lucos_loganne#555 (PR #564).
