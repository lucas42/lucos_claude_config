...
- **State-server module split (#307, PR #308/#309)**: `monitoring_state_server.erl`'s logic now lives in `check_normalisation.erl` (anti-flap gates), `depends_on.erl` (dependsOn semantics — also needs `monitoring_state.hrl`, despite the original proposal naming only 3 of the 4 modules needing it), `alerting.erl` (notify/suppression policy), and `status.erl` (status derivation/output shaping); the server module itself is just gen_server plumbing now. JSON encoding (`encodeStatus`/`encodeInfo`) moved from `server.erl` to `view_json.erl`. Records (`system_state`, `suppression_window`, `pending_verification`) live in `src/monitoring_state.hrl`.
