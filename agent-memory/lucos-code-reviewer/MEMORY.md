...
- [Verify decision attribution before citing it as authority](feedback_verify_decision_attribution.md) — quote the actual decision text before crediting it with settling a scope call; don't infer from the PR's framing (lucos_agent PR #77, caught by team-lead)
- [Check for shared/global side effects outside the diff](feedback_shared_global_side_effects.md) — a value change to a package-level global can silently affect a caller the diff never touches; green tests/0-diff dry-runs don't cover it (lucos_repos#499→#501, self-diagnosed by SRE)
