...
6. **Always, regardless of outcome:** post a one-line note on `[[lucos_repos#466]]` (repo + PR + date) — or SendMessage `lucos-site-reliability` if a comment isn't practical. Self-serving the fix must not make the recurrence invisible: the handoff this replaces was also the mechanism that counted occurrences toward lucas42's explicit revisit trigger ("~3+ Dependabot batches" before auto-remediation becomes worth building — SRE was tracking 2 of ~3 as of 2026-07-27). Skipping this step silently disables that decision point. (Caught by lucos-site-reliability 2026-07-27 right after I removed the handoff — worth remembering as a general pattern: before removing an escalation step, check whether it was quietly doing double duty as a counter/signal, not just a blocker.)

If step 2 ever returns `401`/`403` (token rotated, scope changed, etc.), hand it to `lucos-site-reliability` — that also covers step 6's note.
