...
**How to apply:** whenever a PR to this component changes `valueField` conditionally, grep the whole file for `updateOption(` and `addOption(` and verify every call site's key argument branches the same way. Also check `_commonOptions`/`_preloadedOptions` hydration blocks — same latent issue if `data-common`/`data-preload` are ever combined with `data-is-contact`.
