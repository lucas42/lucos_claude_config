...
- #306: Incident runbook Scenario B doesn't account for serve-stale consumers exceeding ≤35-min bound (raised 2026-07-09, Low, non-blocking) — **CLOSED 2026-07-10 via PR #307, APPROVED by me** after SRE fixed all three findings (verified against actual re-fetched file content, not just the changelog): `lucos_creds_ui` correct in table + command with inline caveat, `lucos_arachne` split into `lucos_arachne_mcp`/`lucos_arachne_explore`, timing bumped to ~35 min margin. Threat-model/mitigation design (blanket restart-after-window, cold-start-fail-closed argument) confirmed sound by independently re-deriving it from the actual `createServeStaleJWKS`/`isJWKSInfraError` code (creds#447), not just trusting the design description. See [[lesson-verify-real-client-behavior]] for the generalised "verify runbook container names against the real docker-compose.yml" lesson.
...
**Tracked and recommendation posted (2026-07-10, lucas42/lucos_aithne#305):** recommended **both**, unequal weight. Audit logging is the load-bearing control — the enumeration space is small enough (bounded principals × small public scope vocab) that any rate limit generous enough not to block legitimate bulk-verification use also lets a patient attacker finish enumerating within an hour; only a log (reusing #161's existing admin-action-logging convention, already applied to grant create/revoke) actually lets an operator notice the pattern once `aithne:read` broadens. Rate limiting recommended anyway as cheap defense-in-depth, reusing the existing tested `keyedLimiter` from `ratelimit.go` (#160) — keyed on the **calling principal** (not IP, since this is an authenticated route), generous limit (~60/min, matching `tokenEndpointLimit` precedent). Told lucas42 this doesn't need architect input — both pieces are direct reuse of already-adopted repo patterns applied to one more endpoint. Decision is his; once made, routes straight to implementation.

**CLOSED 2026-07-10 via PR #308, APPROVED by me** after reading all three diffs directly (main.go, ratelimit.go, main_test.go), not the PR description: rate-limit check correctly precedes the DB lookup; audit log correctly fires only on a genuinely completed query (not on 400/429/500 branches) — but checked specifically whether a 429 leaves a blind spot right where the tripwire matters most, and it doesn't: the rate-limit-exceeded branch has its own distinct log line (principal + count), so a burst is still visible even without the full principal_id/scope/granted detail. Per-principal (not global/IP) keying is genuinely proven by `TestAdminGrantsCheck_RateLimitIsPerPrincipalNotShared` (two distinct tokens, confirms independence), not just asserted. `claims.Subject` attribution reuses the pre-existing `claimsContextKey` pattern already used in `main.go`/`enrolment.go`/`machine_credentials.go`.
