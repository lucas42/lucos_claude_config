...
## #277 shipped via PR #280 (reviewed + APPROVED 2026-07-06) — gap CLOSED

Same-day follow-up to #279. `token.MintIDToken` gained a `scopes []string` param
(same `ClaimScopes`/nil-normalisation as `MintSession`), fed with the same
`effectiveScopes` already computed in `handleAuthCodeGrant` — no second intersection
implementation. `handleUserinfo` echoes `claims.Scopes` (already extracted by
`ParseSession` from the presented access token). `claims_supported` discovery doc
updated. Zero-grant-principal tests confirm issuance stays ungated (ADR-0001 §6).

**The original gap this file describes (id_token/userinfo carry no scopes claim) is
now closed.** A generic OIDC RP (oauth2-proxy etc.) can gate on `scopes` via either
the id_token or `/oauth2/userinfo` as of this PR. The scope-enumeration side-channel
note above still applies (userinfo/id_token scopes are just an echo of what the
RP's own access token already encodes — not a new exposure), but there's no longer
an aithne-side prerequisite blocking a static-SPA-behind-oauth2-proxy design.
