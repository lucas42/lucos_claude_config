...
- [lucos_aithne_jsclient security history](lucos-aithne-jsclient-adr-review.md) — ADR-0001 (2026-07-10): kid log-injection, loginUrl open-redirect, verifyToken() scope-gate, all APPROVED after fixes landed (feb511c/ae8ff03/cc2933f). PR #10: `appOrigin` config fixes dead same-origin returnUrl check; 4-consumer rollout COMPLETE (creds#453 my sign-off, notes#464/seinn#559/loganne#568 code-reviewer solo). PR #12: `_setVerifier` runtime setter removed (construction-time-only `_verifyFn`) — verified no consumer calls it outside tests, so future un-migrated adoption fails loud not silent.
