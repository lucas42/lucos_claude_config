...
- **Accepted-risk precedent doesn't transfer between sibling repos on exposure profile alone** — a past example was `lucos_contacts_fb_import#17` (accepted: local, one-off, user-run script logging clear-text) vs. `lucos_contacts_googlesync_import` (issue #218, raised 2026-07-09; logs the same kind of data but runs as an always-on production cron container). fb_import is now decommissioned (lucas42/lucos#271, archived) so that specific comparison is moot, but the general rule stands: check each repo's own exposure profile (local/one-off vs. always-on/production) before assuming an acceptance elsewhere covers it.
...
- [lucos_mail SASL hashes committed to public repo, years-unrotated](risk-lucos-mail-credential-hashes-in-repo.md) — lucos_mail#75 (2026-08-08), Medium, public issue not advisory.
- [Don't infer credential environment from sensitivity](lesson-dont-infer-environment-from-sensitivity.md) — check actual lucos_creds system/environment/name before asserting "this needs lucas42"; caught on lucos#286.
...
- [lucos_aithne_jsclient security history](lucos-aithne-jsclient-adr-review.md) — ADR-0001 (2026-07-10): kid log-injection, loginUrl open-redirect, verifyToken() scope-gate, all APPROVED after fixes landed (feb511c/ae8ff03/cc2933f). PR #10: `appOrigin` config fixes dead same-origin returnUrl check; 4-consumer rollout COMPLETE (creds#453 my sign-off, notes#464/seinn#559/loganne#568 code-reviewer solo). PR #12: `_setVerifier` removed at library layer (construction-time-only `_verifyFn`). lucas42/lucos#268 (my finding, consumers re-opened the footgun one layer up): RESOLVED — architect's fuller shape (`{middleware}` + `verifySessionToken` only where websocket needs it) adopted across all 4, creds#455 shim closed unmerged/superseded by creds#456 (APPROVED).
