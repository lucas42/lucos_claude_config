...
- [lucos_aithne_jsclient ADR-0001 review](lucos-aithne-jsclient-adr-review.md) — REQUEST_CHANGES on PR #1 (2026-07-10): kid log-injection gap, loginUrl open-redirect enforcement, verifyToken() scope-gate split. APPROVED at feb511c after all three landed — verify v1 implementation PRs match the ADR's spec. v1 code (PR #6, ae8ff03) reviewed + APPROVED: all three confirmed correctly implemented and tested against real crypto/error shapes, not mocks. Re-approved at cc2933f after code-reviewer's 2 findings fixed; `_setVerifier` runtime-setter judged not-blocking (recommend construction-time-only hardening as fast-follow, verify it lands if picked up).
