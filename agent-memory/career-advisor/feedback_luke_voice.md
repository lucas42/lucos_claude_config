...
- **Multi-word noun phrases functioning as compound adjectives — no hyphenation.** Luke prefers these as noun-noun constructions, treating the noun phrase as-is rather than hyphenating it into a compound adjective. Examples:
  - "Principal Engineer" (job title): "Principal Engineer technical direction", not "principal-engineer-level technical direction". Stated 2026-05-21 during a hospitality-marketplace Staff IC draft review.
  - "system architecture": "system architecture responsibility", not "system-architecture responsibility". Stated 2026-05-28 during a Director-of-Architecture CV draft review.
  
  If the construction reads awkwardly without the hyphen, restructure so the noun phrase has its own grammatical seat (e.g. its own subject, or modifying a different noun) rather than adding the hyphen back. This rule applies to noun-phrase modifiers; standard compound adjectives built from non-noun-phrase combinations (e.g. "cross-functional", "long-term", "well-known") are unaffected.
- **"signals"** (when referring to a job advert's contents). Luke's stated take 2026-05-26 during an advisory-consultancy DevEx cover-letter review: doesn't like referring to "signals" in the context of what a job ad contains. Replacements: "the mix it calls for", "what it asks for", "the things named in it", "what you're looking for". Use these constructions to introduce JD-derived bullet lists in cover-letter para 1 openers.
