...
3. Prefer **distroless/static over scratch** for new Go services unless the service provably needs nothing outbound and no timezone handling — "makes no outbound calls" is true at creation and goes false later, silently. scratch's only genuine merit is absolute-minimum attack surface. Still sound as a *per-service recommendation* when a base image is being chosen. **But the formal convention-default (lucos#242) was decided NOT NOW by lucas42 (2026-06-12) — parked, not rejected. Do NOT draft the ADR or re-propose it unprompted.** Immediate gap is covered by the one-time audit lucos#240 ("scratch + outbound HTTPS ⇒ COPY CA bundle"), which is proceeding independently.
