...
**Sibling incident (2026-07-30, lucos_worlds#61 — the "it can't be done" variant):** at #42 I wrote, into a shipped CSS file header *and* the issue body, that BookStack's dark-mode toggle "can't be removed — BookStack has no server-side setting to disable it, and hiding the button would strand a user whose preference is already persisted". Both halves were true; the conclusion was not. I'd reasoned from the *persistence mechanism* I happened to be looking at and never searched the **view-override surface** — `resources/views/common/dark-mode-toggle.blade.php` is a single self-contained partial included from all five toggle sites, and BookStack's theme system prepends the theme dir to the view finder, so an empty override removes every instance with no patch. The stranding objection was real but *mitigable* (clear the stored preference in a `custom-cont-init.d` script), not fatal. **An impossibility claim about a third-party app is an absence claim** — "there is no supported way to do X" needs the same exhaustive search as "nothing consumes Y", across *every* extension surface the app offers, not just the one the problem first presented on. And impossibility claims are stickier than ordinary errors: this one got baked into a source file and stood for three weeks.

Trigger: writing "has no other home", "there's no existing pattern for this", "nothing already does X", "there's no way to do X" — **especially when it's the reason you're adding a table, column, endpoint, or service, or the reason you're ruling an option out.** Grep first; for adopted apps, enumerate the extension surfaces before ruling anything out.
