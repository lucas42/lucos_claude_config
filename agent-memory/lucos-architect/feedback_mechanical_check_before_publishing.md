...
**Why:** Two failures in the same 24h window (May 2026):
...
Both failures are the same shape: I had a memory-derived claim about a finite enumerable set (refs, repos), and didn't run the cheap mechanical check that would have confirmed or refuted it.

**How to apply:**

- **Before publishing any ticket / ADR / PR body that names a set of items, identify the authoritative source-of-truth for that set and check against it.** For arachne emitter audits: `live_systems` in `triplestore.py`. For cross-repo issue references: a `grep -nE "<this_repo>#|<other_repo>#"` against the prose. For migration callsites: a `grep -nE "<old_pattern>"` against the producer code.
