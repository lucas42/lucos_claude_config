...
- **The trap also fires on a *tangential, non-load-bearing aside* — and that's where the discipline most often lapses.** On 2026-06-28 I wrote "(tangential observation, no action asked: backups#352 / creds#415 are still open, so lucos_authentication#143's blocker isn't cleared)" into a `lucos#251` comment — both had in fact closed/completed hours earlier (06:23Z, 08:09Z); I'd raised them earlier *in the same session* and asserted their state from memory. The aside felt too minor to verify, which is exactly why it skipped the re-fetch — and it was wrong, sending team-lead a false premise. **A live-state claim costs a re-fetch whether it's the headline or a parenthetical; if it's not worth re-fetching, it's not worth asserting — drop it instead.**
