...
**Generalised (2026-06-08, lucos_backups#307):** the same "check history first" applies to inferring **why a config setting exists**, not just to SSH changes. I claimed `lucos_backups`' `network_mode: host` was vestigial after reading the *current* connection code (aurora via ProxyJump → no mDNS needed in-container). lucas42 corrected me from `git log`: commit `bb81f37` added it for **IPv6 outbound to salvare**. I theorised purpose from one plausible code angle without checking the commit that introduced the setting — and ADR-0007 §Context had even documented the real reason. **Rule: before asserting why an infra setting (network_mode, a flag, an env var, a port) exists or is safe to remove, `git log`/`git blame` the line that introduced it and read the commit message.** A current-code reading tells you *what*; only the introducing commit tells you *why*. (My runtime-verification gate caught the dep before any change, but the reasoning should have been right first time.)
