...
**The principled split** (settled in the rework): **Loganne is the cross-estate async event bus for inter-service communication**; service logs carry local execution detail. **Updated framing 2026-05-05 (lucos_scenes#52):** "cross-estate state changes" was the original framing but it's too loose — it lets observability-only events drift in. The accurate frame is that Loganne carries events *that other services need to act on asynchronously, or that have a concrete external consumer*. Some events are pure observability (consumed only by aggregators); others drive downstream behaviour (`trackUpdated`/`trackDeleted` driving state in media_manager and media_weightings; `monitoringAlert`/`monitoringRecovery` driving alerting). The test for "should X be a Loganne event?" is **name the consumer that needs this asynchronously** — not "is this an interesting cross-estate happening?"

A `console.log` in seinn for per-track transitions is fine; a Loganne `playbackTransition` event for the same thing isn't. The distinction is consumer demand, volume tolerance, and audience — not the literal level of detail.
