...
- v3 ideation (#45): API structural changes DECIDED: rename `trackid` -> `id`, remove debug weighting fields, structured JSON errors, richer pagination. Weighting endpoint stays plain text (lucas42 rejected). lucas42 then raised much larger data modelling scope:
  ...
  - lucas42 DECIDED (2026-03-10): tag value format approved. Field name: `name` (not `value` or `label`), consistent with lucos_eolas. Data sync: belt-and-braces (Loganne webhook + periodic reconciliation). Deletion: architect recommended Option A (clear URI, keep name).
  ...
  - **Post-v3 tickets filed (2026-04-10)** from #45 review: #136 (write validation for RequiresURI), #137 (first-class album), #138 (data sync webhook+reconciliation), #139 (deletion handling Option A), #140 (controlled vocab migrations). Merge tooling: lucos_eolas#157.
  - **#137 design (2026-04-11):** Albums are both a first-class type AND a tag. New `album` table (`id`, `name` -- deliberately minimal, no art_url), new endpoints `GET/POST /v3/albums`, `GET /v3/albums/{id}`. Track→album relationship stays in the existing `album` tag, using the v3 tag value object pattern: `{"name": "Abbey Road", "uri": ".../v3/albums/42"}`. No schema change to tag table -- `value` column stores album id as string for `predicate='album'` rows; API joins on read. Write side requires `uri` for album tags (structured error if omitted). Rollout: single coordinated deploy (joint API + UI), not three phases -- lucas42 pushed back on over-engineering the rollout given small user base. Companion UI ticket: `lucos_media_metadata_manager#202`.
...
- **Triplestore config (current):** TDB2 disk-backed storage. Two Fuseki endpoints: `raw_arachne` (read-write) and `arachne` (read-only, **no reasoner**). Inference removed; transitive closures pre-computed by the ingestor and written to `urn:lucos:inferred` graph. Confirmed in `triplestore/configuration/arachne.ttl` (May 2026). Implication: `owl:inverseOf` declarations don't materialise reverse triples — safe to use as label metadata.
- **Memory issue (#86, resolved):** Originally OOM-killed twice on avalon under OWLMicroFBRuleReasoner with 2GB container limit. Resolved by Option 3 (pre-compute transitive closures in ingestor, remove OWL reasoner entirely). Fuseki is now a simple SPARQL store.
