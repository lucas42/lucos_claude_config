...
6. **This applies just as hard to option lists *I* author.** When I enumerate (a)/(b)/(c) myself, the shared premise across all of them is the thing most likely to go unexamined — precisely because I never see it as a "choice." Before building options on a constraint, trace it to its **source of truth** and check it's actually a requirement, not an inherited assumption: an ADR clause is a requirement; a *caveated review recommendation* ("X is acceptable only as a conscious threat-model decision") is not. Watch especially when the shared constraint is what makes the problem hard or the solution elaborate — that's the one to challenge first, because dropping it may dissolve the whole problem. (2026-06-23, lucos_aithne#191: I built three options — configy-derived / cors_origins table / union — all assuming "explicit membership, not a `*.l42.eu` glob", and recommended a new DB table + admin endpoints + audit. lucas42 challenged the premise: ADR-0003 line 32 *specifies* `*.l42.eu` origins; the no-glob line was only security's caveated ADR-review note, which **I had relayed myself** on #181. The glob — which I never evaluated — obviates the entire population-path problem and the table. Self-verification #5 "would a simpler solution serve?" and #6 frame-review both should have caught it.)
