...
- `lucos_scheduled_scripts` specifics: it's a base image (`FROM lucas42/lucos_scheduled_scripts:<tag>`) with downstreams `lucos_arachne` (ingestor), `lucos_dns` (sync), `lucos_media_weightings`, `lucos_backups`, `lucos_contacts_googlesync_import` — most of which already declare the loganne/schedule_tracker clients in their own Pipfile, so the base image's unpinned `pip install` of them is largely redundant (and non-reproducible). See also [[check-protocol-contract-before-accepting-break]] (ADR-0011, the consumer-side counterpart).
