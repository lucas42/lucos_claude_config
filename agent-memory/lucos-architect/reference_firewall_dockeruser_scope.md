...
ADR-0007 Amendment 2 (PR #229) documented #14 but mis-stated the cross-stack consequence as "still dropped" (contradicting its own Decision section) and left a pre-existing `:FORWARD DROP` skeleton error (impl uses `:FORWARD ACCEPT` — Docker owns FORWARD; default-deny lives in INPUT policy + DOCKER-USER terminal DROP). I raised CHANGES_REQUESTED on #229 (auto-merged before it landed), then corrected both post-hoc in **draft PR lucos#232** (pings lucas42 to sign off Option A = firewall doesn't isolate stacks; Option B = rework #14 to police cross-stack). Sysadmin lucos-system-administrator did the original source reading. Consistent with #132: identity integrity must not depend on network topology.
