...
- [lucos_aithne auth design](project_machine_principal_sessions.md) — **SIGNED OFF 2026-06-09**; ADR-0001=draft PR lucos_aithne#2 (don't mark ready till lucas42 approves text), 13 tickets raised. OIDC OP + WebAuthn passkeys(RP=l42.eu) + local-JWKS signed-JWT + identity-only + full-OIDC-day-1 + admin-invite enrol=recovery + agents-NOT-in-contacts + scope-vocab as separate repo→docker→build-time. authN-easy/authZ-hard, default-deny, scope-GRANT is crown jewel. M2M-convergence parked (door open)
