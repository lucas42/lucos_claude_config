...
- Auth origin = env-varying `AITHNE_ORIGIN` env var (dev vs prod aithne), injected per aithne#148. (Supersedes old hardcode-`auth.l42.eu`-in-app-code note; `lucos_authentication` decommissioned 2026-06-29.)
...
- [Alertable check must be able to recover](feedback_alertable_check_must_recover.md) — monotonic "since restart" counter is wrong for an alert; anchor the clear to the real heal event (reconcile), not a redeploy (lucas42, arachne#702, 2026-06-30)
...
- [Check shared failure domain before diagnosing a "two-paths" split](feedback_check_shared_failure_domain_before_diagnosing_split.md) — divergent answers from two endpoints may be probe-timing, not resilience, if both share a failure domain (#410, 2026-06-07)
- [Crossed-message thrash: let the decisive event settle it](feedback_crossed_message_thrash_let_decisive_event_settle.md) — fast crossed-message loop over a reversible choice: re-verify each turn, hold, converge on the irreversible event (2026-06-08)
...
- [Quiesce-during-read backup pattern](reference_quiesce_during_read_backup.md) — engine-agnostic `docker pause` around LOCAL read; crash-consistency; guaranteed-unpause is dominant risk (backups#344)
- [navbar keepalive vs consumer service workers](reference_navbar_keepalive_sw_interception.md) — cross-origin remint POST intercepted by SWs; exempt aithne origin/`/auth/*` (not blanket); only notes+seinn affected (navbar#180)
- [Convention catalogue + enforced-vs-guidance boundary](reference_convention_catalogue.md) — lucos_repos ADR-0007 MERGED; generated `docs/conventions.md` + golden-file test; docs LINK the catalogue, never paraphrase enforced rules; docs refactor = claude_config#120
...
- [aithne migration guide](project_aithne_migration_guide.md) — ADR-0003 MERGED (aithne#182, closed #147; security obs→#181/#174); guide MERGED (aithne#186, closed #159); deferred impl aithne#181 + navbar#174; pending guide-hardening: `next=` use-current-path nudge (batch w/ navbar#174 version patch)
