...
- **Incident 2026-04-17** (`docs/incidents/2026-04-17-docker-mirror-overload-and-orb-publish-bug.md`): orb rollout #118 had two stage failures -- `docker tag` after buildx docker-container driver doesn't work (need `docker buildx imagetools create`, fixed in orb #120), then parallel re-trigger of 8 pipelines overloaded the mirror. Three visually distinct errors (`context deadline exceeded`, `manifest unknown`, BuildKit `unexpected commit digest`) all from one root cause: mid-stream blob truncation. Follow-up: orb #122 (graceful fallback when mirror unavailable).
