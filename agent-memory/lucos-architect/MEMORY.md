...
- People modelling (#19): Option C (split by type) agreed. Names deferred. Contact linking owned by lucos_contacts (`eolas_uri` field). Search: `is_contact` boolean (not `source`). Merge logic in arachne **search-index ingestor** (NOT triplestore — explicit layer split per lucas42 2026-05-17). Primary `id` decided by walking asymmetric RDF predicate `https://eolas.l42.eu/ontology/preferredIdentifier` (NOT hardcoded "eolas-wins" — data-driven, source systems declare their own preference). Contacts emits both `owl:sameAs` (symmetric, for SPARQL) AND `preferredIdentifier` (asymmetric, points at canonical URI). Reconciliation: `personMerged` loganne event (emitted by **lucos_contacts**, not arachne — contacts owns the merge relationship) + lazy `id = X OR X ∈ secondary_uris` lookup. Tickets filed 2026-05-17: eolas#253 (declare predicate), contacts#712 (emit + event), arachne#539 (search-index merge w/ predicate walk + explicit layer split), search_component#165 (data-is-contact filter), mmapi#236 (subscribe + rewrite). lucos_photos does NOT need ticket — stores numeric `contact_id`, not URI.
- **media_metadata_api consumer status (corrected 2026-05-17):** v3 cutover IS DONE. v2 returns 410 Gone (`v2_gone_test.go`). All consumers reference v3 (weightings, manager Java + PHP, import). What's still open from #45 ideation backlog: #138 (data sync webhook+reconciliation), #139 (deletion handling Option A), #140 (controlled vocab migration to eolas URIs — blocked on eolas#75 write API). So `personMerged` doesn't have a live consumer *yet* not because v3 is parked, but because #140 hasn't started. Once #140 lands, the consumer is live. Correct the eolas#19 caveat at body-rewrite time.
