...
- People modelling (#19): Option C (split by type) agreed. Names deferred. Contact linking owned by lucos_contacts (`eolas_uri` field). Search: `is_contact` boolean (not `source`). Merge logic in arachne ingestor via `owl:sameAs`; **eolas URI wins as primary `id`** in merged records (first-seen rejected on clock-skew grounds, 2026-05-15). Reconciliation: `personMerged` loganne event (emitted by **lucos_contacts**, not arachne — contacts owns the merge relationship) + lazy `id = X OR X ∈ secondary_uris` lookup. Event name `personMerged` (not entityMerged/contactMerged) — accurate for cross-system identity, future-proof if eolas later merges duplicates. Follow-ups to file once URI rule signed off: contacts (`eolas_uri` + RDF + event emit), arachne (merge logic + secondary_uris + is_contact), search_component (`data-is-contact` filter), media_metadata_api (subscribe + rewrite). lucos_photos does NOT need ticket — stores numeric `contact_id`, not URI.
