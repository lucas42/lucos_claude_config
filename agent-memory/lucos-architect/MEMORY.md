...
- **Relationship deletion (#53, 2026-05-07):** lucas42 rejected my asserted-vs-inferred split (option A) on ethical grounds — all five inferred cousins are equally cousins; can't class-split. ADR-0001 committed: deletion refused iff row would be re-inferred from remaining graph; inverse cascade falls out for free; sibling-propagation via a "delete across sibling group" composite UX primitive. PR #698 implemented but closed for journey breakage (form action, duplicate h1, multi-confirmation cascade). Split into #699 (test harness only — pre-#698 admin behaviour), #700 (admin journey, reintroduces #698-shape code with GET-time decision), #701 (engine refactor + ADR-0002, single engine + DeletionPlan). PR #702 was a re-land attempt — closed. See [feedback_test_harness_assertions_reachable.md](feedback_test_harness_assertions_reachable.md) for the body-vs-intent contradiction lesson.
