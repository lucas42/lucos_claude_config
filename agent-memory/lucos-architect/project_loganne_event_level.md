...
**Recommended taxonomy (lucas42 to sign off):** two levels — `detail` (hidden by default) < `normal` (default, shown). Recommended NOT seeding a higher `notable` tier (dead threshold today; adding higher later is non-breaking).

**Follow-ups to raise after sign-off:** lucos_monitoring (suppressed-window alerts → low level); track-finished/lucos_media_weightings emitter (→ low level).
