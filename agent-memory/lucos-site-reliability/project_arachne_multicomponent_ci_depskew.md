...
**dependabot.yml dir-form convention (bit me 2026-06-17, PR #653):** the `docker-dependabot-updater-present` convention derives expected docker dirs from `docker-compose.yml` `build:` paths VERBATIM, incl the leading `./` → a `build: ./mcp` requires dependabot `directory: /./mcp` (NOT `/mcp`). `build: search` (no prefix) requires `/search`. So arachne docker dirs are MIXED: `/./ingestor /./triplestore /./web /./mcp` (./-builds) + `/search /explore` (plain builds). pip/maven/npm dirs are PLAIN — those conventions derive from the manifest file's directory, not compose build paths (so `pip /mcp`, `pip /ingestor`, `maven /triplestore`, `npm /explore` are all plain). DON'T "normalise" `/./x`→`/x` — it breaks convention-check. Check `docker-compose.yml` build paths before touching docker dependabot dirs.
