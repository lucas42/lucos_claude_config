...
- ✅ **The review OBJECT persists** through *all* of: convert-to-draft, ready-for-review, close, and reopen — same `commit_id`, still `APPROVED` (verified on `lucos_worlds#75`, one approval surviving four state transitions). Don't say "the approval is lost"; it isn't.
- ⚠️ **What's lost is the armed `auto_merge`** — that needs a fresh `pull_request_review: submitted` event to re-arm. Those are different things and only the second needs redoing.
- ✅ **Arm auto-merge EARLY — approve before the check lands.** `gh pr merge --auto` waits; it cannot merge ahead of a required check. On `#73` auto-merge armed at 23:24:55Z, `Analyze (python)` went green 23:25:49Z, merged 23:25:57Z. So approving while checks are pending saves a whole webhook round-trip. (⚠️ This *inverts* the right advice during heavy throttling, where "approve → verify the run appeared → re-approve" is correct because delivery is the scarce thing. Same action, opposite ordering — pick by which resource is scarce: delivery, or wall-clock.)
...
- **CodeQL / any workflow with `workflow_dispatch:`** — dispatch it on the PR's branch. Needs `actions:write` → `lucos-system-administrator`. ⚠️ **It POSTS the check but does NOT necessarily COUNT it. These are different claims and only the first is evidenced.**
  ...
  - **Cause undetermined, and a later observation made it MORE open.** Comparing a dispatched PR against a never-dispatched one on the same repo/day (`lucos_worlds#72` vs `#73`):
    ...
    |---|---|---|
    | 7 × `ci/circleci:` | present | present |
    | `CodeQL` *(github-advanced-security)* | **absent** | **present, SUCCESS** |
    | `Analyze (python)` *(github-actions)* | absent | **absent** |

    So the dispatch **did** add one rollup entry (the Advanced Security one) and **not** the required GitHub-Actions one. That undercuts a simple "dispatch-triggered runs never join the rollup" rule — one of the two suites from that same dispatch joined fine. Equally consistent with partial/degraded propagation. **Don't assert either cause**; note that a clean control needs a non-outage day.
  - ⚠️⚠️ **`statusCheckRollup.state: SUCCESS` does NOT mean "required checks satisfied".** It reflects only *what is in the rollup* — an **absent** required context doesn't fail it, it simply isn't counted. So a PR can show rollup SUCCESS + `reviewDecision: APPROVED` + `mergeable: MERGEABLE` and still be `mergeStateStatus: BLOCKED`. **Assert presence of each required context by name; never read the aggregate state as satisfaction.** (team-lead read SUCCESS as a disconfirmation and nearly closed the investigation on it — 2026-08-06. Likewise `check_suites[].pull_requests: [N]` proves suite↔PR association, *not* that the context entered the rollup.)
  - **Narrowing `BLOCKED` to a single cause** — read `/protection` and eliminate: on lucos_worlds/main every other gate was off (`strict:false`, `required_conversation_resolution:false`, `required_linear_history:false`, `required_signatures:false`, `lock_branch:false`, `block_creations:false`, `enforce_admins:false`) and `reviewThreads` was empty with `reviewDecision: APPROVED`. With all other branches eliminated, the missing context is nameable as *the* cause rather than a guess. `strict:false` in particular means a moved base is NOT a blocker — don't chase "branch is behind".
