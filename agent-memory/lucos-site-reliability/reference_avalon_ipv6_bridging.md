...
- `lucos_monitoring_default` (172.22.0.0/16) and `lucos_time_default` (192.168.16.0/20) = live **EnableIPv6=false**, so they cannot do IPv6 egress today. ⚠️**2026-08-08: this is UNAPPLIED CONFIG, NOT INTENT — do not cite it as a decision (I did, on lucos#278, and was wrong).** Both compose files have declared `enable_ipv6: true` since **2026-05-22** (commit `5e98dc7`, ADR-0007) and it has never taken effect: Compose attaches to an existing network and does not retrofit changed attrs, and both networks were created **2024-04-28** and never recreated (their subnets predate the daemon's current `default-address-pools`). Same for `lucos_dns_secondary_default` on xwing. `lucos_dns_default`/`lucos_backups_default` DO carry IPv6 — both created 2026-06-08, i.e. after their config changed. Detection gap tracked as lucas42/lucos#279 (my recommendation: deploy-time compare in `lucos_deploy_orb`, the only place holding both the compose file and a Docker connection to the host — prod hosts have no compose file). **Rule: `docker network inspect` tells you live state; it does NOT tell you what was intended. Read the compose file too before calling anything deliberate.** (Architect also wrongly claimed on #307 that monitoring fetches salvare — that part was false; corrected.)
