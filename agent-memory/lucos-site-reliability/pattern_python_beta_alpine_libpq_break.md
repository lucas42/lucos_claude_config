...
**⚠️ STALE-CLAIM CORRECTION (verified 2026-07-31):** the note below that "prod already runs a8 pre-release" is **no longer true** — `lucos_contacts` and `lucos_eolas` Dockerfiles are now on stable `python:3.14-alpine`, and `lucos_media_import` on `python:3.14`. Don't cite "the estate deliberately runs pre-release python in prod" as a *current* fact; it was true in June, it isn't now. It remains valid as evidence that a blanket estate-wide pre-release ban needs an exemption path. See [[pattern-baseimage-bump-runtime-break]] for the 2026-07-31 php-alpha instance and the estate defence inventory (lucos#273).
