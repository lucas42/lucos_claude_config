...
**The weekly `import.py` full scan IS a backstop, but a weak one** — it recovers a missed file only when a run actually completes, and completion is intermittent (last success 2026-07-30 13:01:09Z; the 08-06 run died — #173). `import.py` iterates `sorted(os.listdir(...))` and Python sorts **uppercase before lowercase**, so the 12 lowercase dirs (`artists`, `bandcamp`, `classical`, `iTunes`, `qobuz`…) are **scanned LAST every run** — least margin, first dropped when a run is killed. So a `new_files` miss there is not permanent, but recovery is unbounded in practice.

⛔ **`/var/state/import_checkpoint.json` is NOT a history of what has ever been scanned.** `completed_dirs` is the progress of the *current unfinished run* only — `clear_checkpoint()` deletes the file on success, so **the file exists only when a run has FAILED**. I misread it as cumulative on 2026-08-07 and published "12 of 27 dirs never reached / 72% of the library unreachable" to two tickets before retracting. **Authoritative source for "has the full scan run?" is `curl https://schedule-tracker.l42.eu/jobs`** → `lucos_media_import/all_files` → `metrics.age` (seconds since last completion) + `metrics.errors`. One HTTP call. [[feedback_verify_state_file_semantics_before_reading_history]]
