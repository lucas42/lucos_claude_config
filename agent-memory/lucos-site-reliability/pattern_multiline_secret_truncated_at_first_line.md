...
**Length + framing + no-CRs does NOT mean valid.** A key can be full-length, LF-only, with correct `-----BEGIN-----`/`-----END-----` and zero CRs, and STILL be cryptographically invalid (`ssh-keygen -l -f` → "not a key file"; SSH client load → `error in libcrypto`). Never infer validity from length or header/footer; run `ssh-keygen -l -f` (or a test SSH). A "fix" (re-store, line-ending conversion) can swap one corruption (CRLF-truncation) for another (full-length-but-invalid). **Fixing a stored credential is not done until the stored value is validated with the real tool** — and against the EXPECTED fingerprint, not just "some valid key". Discriminating test for transport-vs-store: check a sibling key on the SAME deploy (e.g. `lucos_creds_ui`'s key) — if it's valid, transport is fine and the specific stored value is corrupt.

**Root cause of "full-length-but-invalid, no CRs": a stripped trailing newline — classically `bash $(...)` command substitution.** An OpenSSH key ends `-----END OPENSSH PRIVATE KEY-----\n`; bash `$(cmd)` / `$(cat file)` strips ALL trailing newlines from the captured value, so the final `\n` silently vanishes. Result: a value that's the right length, correct framing, no CRs, but `ssh-keygen`-invalid + `error in libcrypto` on load. **The only reliable tell is `ssh-keygen -l -f` (invalid) — NOT a byte-count heuristic.** ⚠️ Do not use "env value is 1 byte longer than the on-disk key" as the signature: I tried that and it's WRONG — the container serialization adds a constant ~1-byte env>disk delta to BOTH the broken key (env 419 / disk 418) AND the fixed valid key (env 420 / disk 419). The defect is that the broken key is 1 byte shorter *than a known-good key of the same type* (disk 418 vs 419), which you can only see by comparing to the good length/fingerprint — not from the env/disk delta. Fix: write the key byte-for-byte with NO command-substitution capture (heredoc, direct file copy, or a byte-safe script), and validate the stored value's fingerprint against the expected one before deploy. (2026-07-22, #474 second-phase root cause, found by sysadmin in the dev store; verified fixed in prod — fingerprint matched + real sync completed.)
