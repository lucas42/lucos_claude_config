...
- **Data:** `~lucos-agent/emergency-backups-2026-09-14/` on **xwing (original) and salvare (copy)**, both mode 700, sha256-verified. **`README.md` there is the restore guide:** which file per volume, how each copy was taken and verified, what wasn't copied, and checksums.
  ...
  - **SSH host keys (PRIVATE):** `rescue/avalon-ssh-host-keys/`, copied 2026-09-15 at lucas42's request. Whether to reuse them depends on lucos-architect's advice on #296 (does the rebuild keep the hostname?). **If the decision is fresh keys, delete the dir on BOTH xwing and salvare.** The fingerprints match clients' known_hosts for avalon.s.l42.eu; drop DSA if reused (OpenSSH 10 can't load it).
