...
- **Branch protection: the limit is the ENDPOINT, not the App.** `/branches/{branch}/protection` needs admin and 403s "Resource not accessible by integration" for most agent Apps (confirmed: `lucos-architect`). But **`/branches/{branch}` embeds the same data** at `.protection.required_status_checks.contexts` and is readable by every agent App tested (`lucos-site-reliability`, `lucos-code-reviewer`, `lucos-architect`). Both endpoints return identical contexts — verified side-by-side on lucos_worlds/main 2026-08-06.
  - So when a teammate says "I can't read branch protection, route it to sysadmin": the fix is **`--jq '.protection'` on `/branches/{branch}`**, not routing to anyone. ⚠️ I initially told two teammates to send these reads to me because *my* App could hit `/protection` — an overcorrection that invented needless routing. Diagnose which endpoint 403'd before concluding it's a permissions gap.
- The two endpoints differ by **schema**, not just permissions: `/protection` returns ~11 keys (`enforce_admins`, `allow_force_pushes`, `required_conversation_resolution`, `lock_branch`…); `/branches/{branch}.protection` returns only 3 (`enabled`, `required_status_checks`, `enforcement_level`). So for contexts either works; for anything else you need `/protection`.
...
- Branch not protected at all → `/protection` 404s `"Branch not protected"` (e.g. the `lucos` repo). That IS a clean, distinguishable signal.
