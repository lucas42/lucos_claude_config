...
- Reading protection settles "is X *really* a required check?" — `required_status_checks.contexts` lists them, and `required_pull_request_reviews: null` means protection imposes **no** review requirement (so "dismiss stale reviews" worries are moot, and approvals gate only via the auto-merge workflow).
