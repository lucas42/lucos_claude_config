...
- [Crash-loop makes docker_health flap 51 alerts/42 recoveries](pattern_docker_health_crashloop_flapping.md) — guard keyed to StartedAt; compare a MONOTONIC counter.
...
- [⚠️`failThreshold` counts SOURCE UPDATES, not failing polls](pattern_failthreshold_counts_source_updates.md) — failThreshold=2 fires on ONE bad reading; #303.
...
- ["0 occurrences in N" needs EXPOSURE, not just absence](pattern_stale_line_race_needs_exposure_not_absence.md) — linuxplayer#143 CONFIRMED; 113/127 changes couldn't trigger it.
- [Optimistic cache of a REMOTE process's state](pattern_optimistic_cache_of_remote_process_state.md) — "wrong at start, fixed by adjusting"; mplayer drops slave cmds until audio init (~0.9s even without the 16MB cache).
