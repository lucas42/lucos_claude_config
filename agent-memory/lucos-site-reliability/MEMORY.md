...
- [Cross-probe flap despite correct dependsOn in coordinated deploys](pattern_deploy_window_boundary_crossprobe_flap.md) — weightings/metadata-mgr cross-probes (dependsOn already set) fire REAL alerts in a media rollout burst: dependency down BEFORE its deploy-window opens (alert ~1min pre its deploySystem event), recovers 1-3min, no AlertSuppressed. Not new/missing edge. lucos_monitoring#286 P3. Don't re-diagnose.
