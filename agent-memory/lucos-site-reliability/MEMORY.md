...
- **Concurrent-merge race condition in calc-version**: if two PRs land on main within seconds, both pipelines fetch before the first has pushed its tag — each computes the SAME next version and only the first push succeeds. Second fails with `! [rejected]  vX.Y.Z -> vX.Y.Z (already exists)`. Observed 2026-04-17 on lucos_notes (two merges 21s apart, both computed v1.0.12). Distinct from the 403 abuse-detection throttle. Fix: re-fetch tags immediately before computing version, or retry on push rejection.
