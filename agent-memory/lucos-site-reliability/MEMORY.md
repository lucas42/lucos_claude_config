...
- [Incremental rsync path fails host-key verification on ProxyJump hop](pattern_incremental_rsync_container_proxyjump_hostkey.md) — `incremental` backup_strategy (#324) rsyncs in a fresh `docker run` container (root, EMPTY known_hosts); `ssh -o StrictHostKeyChecking=no -o ProxyJump=...` covers outer hop (→aurora) but NOT the jump to xwing → `Host key verification failed`/`Connection closed by UNKNOWN port 65535`/exit 255. photos_photos LIVE+BROKEN on v1.1.13 (configy#230 incremental); create-backups red for it is **lucos_backups#327**, NOT new outage (#309 already failed it). Don't re-investigate. Blocks lucos_photos#427.
