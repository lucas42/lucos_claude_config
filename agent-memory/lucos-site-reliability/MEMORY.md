...
- lucos_creds/issues/82 (SSH connection): closed/completed — hostname fix applied. BUT issue #199 (open, priority:low, owner:lucos-system-administrator): SSH resolution to `lucos-creds` still failing from `lucos_creds_ui` despite `hostname: lucos-creds` being confirmed set. Docker DNS may not register hostname as alias on all network configs. Investigate network isolation or use explicit `networks:` alias instead.
...
- Issue #62 (resolved in batch deploy 2026-04-08): `restart: always` added to triplestore/ingestor/search in the batch. Also #256 (depends_on service_healthy) landed in same batch.
- **Incident 2026-04-08 (outage 1)**: batch deploy (#258–#268) caused full outage. Root cause: PR #266 switched from tarball download to Maven in the triplestore Dockerfile, changing `apt-get install -y wget gettext-base` to `apt-get install -y maven gettext-base` — `wget` was dropped from the explicit package list, but the healthcheck still used `wget`. Base image (`openjdk:22-jdk-slim`) was unchanged throughout. Fix: PR #278 adds `curl` to Dockerfile and updates healthcheck. Service manually restored via `docker start`.
- **Incident 2026-04-08 (outage 2)**: PR #267 (*Cache 3rd party ontology files locally in repo*) renamed `systems_to_graphs` → `live_systems` in `triplestore.py`; `server.py` (webhook handler) not updated — `ingest.py` was updated, `server.py` was not. Ingestor crash-looped on startup; web stuck in `Created` via `service_healthy`. Monitoring did not alert — failure occurred during deploy silence window (see lucos_monitoring#135). Fix: PR #280 (3-line rename in `server.py`).
- **Always verify PR numbers from git log before citing them in incident reports.** Commit messages don't include PR numbers — they must be looked up via `gh api repos/lucas42/{repo}/commits/{sha}/pulls` or inferred from PR merge commits. Getting the number wrong means a factual error requiring a correction commit.
...
- **Healthcheck tool by base image**: `nginx:N` (Debian) has `curl` but NOT `wget`. Alpine images have `wget` but NOT `curl`. `openjdk:N-jdk-slim` has NEITHER by default — must install `curl` explicitly. Wrong tool → command not found → container permanently unhealthy → all `service_healthy` dependents stuck in `Created` state. Use `curl --fail -s -o /dev/null <url>` for Debian/nginx/jdk-slim. (lucos_router PR #24 2026-03-13; lucos_arachne triplestore incident 2026-04-08 — PR #278)
- **apt-get install changes can silently break healthchecks**: when modifying a Dockerfile's `apt-get install` line, verify that no tools used elsewhere in the stack (healthchecks, startup scripts) have been removed. The lucos_arachne 2026-04-08 incident was caused by switching build dependencies and accidentally dropping `wget` — the Dockerfile compiled and ran fine, but the healthcheck broke. (incident report: lucos/docs/incidents/2026-04-08-arachne-triplestore-healthcheck-outage.md)
