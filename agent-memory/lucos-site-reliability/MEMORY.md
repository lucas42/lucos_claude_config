...
- **Issue #130 (open) — Concurrent main-pipeline tag conflict / git-vs-Docker drift**: The orb has two bugs that combine. (1) `push-release-tag.yml` has retry logic but it's defeated by CircleCI's default `bash -eo pipefail`: `push_tag_with_retry "v${VERSION}"; push_result=$?` exits the shell on non-zero return before `$?` is captured. Outer loop never runs. Fix: `push_result=0; push_tag_with_retry ... || push_result=$?`. (2) Build order is `calc-version → publish-docker → push-release-tag`. If tag push conflicts and recomputes VERSION to e.g. v1.0.17, the Docker image is already on Docker Hub tagged 1.0.16 — the git tag and Docker tag diverge. So fixing (1) alone turns loud failures into silent drift. Real fix: `serial-group` on `build-amd64`/`build-multiplatform` at each consumer's workflow level. `serial-group` is workflow-level, cannot be applied to orb commands or from inside the orb. Observed 2026-04-18 on lucos_arachne (4/5 concurrent pipelines failed), lucos_eolas (1), lucos_docker_health (1); also 2026-04-17 on lucos_notes.
...
- **Issue #122 (open, P2)**: orb has no fallback when `docker.l42.eu` mirror is unavailable — a mirror outage hard-fails every estate deploy. Fix: probe `/v2/` before login; skip `--config` injection on failure.
...
- **Architecture**: two containers on avalon — `lucos_docker_mirror_web` (gunicorn proxy on port 8038) and `lucos_docker_mirror_registry` (internal distribution registry at `registry:5000`). Web proxies `/v2/` requests through to the registry which pull-through-caches Docker Hub. NOTE the containers are named with `_web` / `_registry` suffixes — not bare `lucos_docker_mirror`.
- **Issue #19 (open, P2)**: gunicorn web proxy collapses under concurrent CI load. Sync workers serialise blob streaming, OOM'd/timed out when 8 parallel estate builds hit it on 2026-04-17. Fix needed: raise workers count, raise `--timeout`, consider async worker class (gthread/uvicorn/gevent) for pure I/O passthrough, set container memory limit, expose saturation metrics in `/_info`.
- **Saturation symptoms to recognise**:
  - `Worker (pid:X) was sent SIGKILL! Perhaps out of memory?` in web container logs.
  - `WORKER TIMEOUT` after 30s mid-blob in web container logs.
  - Registry logs: `write: broken pipe` and HTTP 500 after tens of MB written.
  - External `docker.l42.eu/_info` times out at 10s but internal `curl http://localhost:8038/_info` on avalon returns instantly → confirms worker starvation, not network/TLS.
- **Three distinct CI-side symptoms ALL from the same mirror saturation**:
  1. `Error response from daemon: Get "https://docker.l42.eu/v2/": context deadline exceeded` at `Docker Login (mirror)` step.
  2. `failed to compute cache key: ... unexpected commit digest ...: failed precondition` — BuildKit received a truncated blob (worker died mid-transfer) and the hash doesn't match the manifest. Commonly looks like a corrupted mirror cache but it's just a partial transfer.
  3. `manifest unknown` 404s.
  Don't debug these as three bugs — they're one capacity problem.
- **Quick remediation**: `ssh avalon.s.l42.eu docker restart lucos_docker_mirror_web` clears stuck workers immediately. Service responsive within seconds.
- **Sequential re-triggers work fine** when mirror is otherwise healthy — 6 consecutive builds completed in ~17 min on 2026-04-17 post-restart. It's specifically concurrent load that kills it.
- Incident report: `docs/incidents/2026-04-17-docker-mirror-overload-and-orb-publish-bug.md` (PR lucas42/lucos#92).
