...
- **Incident 2026-04-08**: batch deploy (#258–#268) caused full outage. Root cause: Fuseki 6.0.0 base image (`openjdk:22-jdk-slim`) lacks `wget`; triplestore healthcheck failed permanently; web/ingestor/mcp stuck in `Created` due to `service_healthy` dependency. Fix: PR #278 adds `curl` to Dockerfile and updates healthcheck. Service manually restored via `docker start`.
