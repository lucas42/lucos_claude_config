...
**Per-check config lives in /_info, not lucos_monitoring.** See [feedback_failthreshold_lives_in_info.md](feedback_failthreshold_lives_in_info.md). `failThreshold` and any equivalent per-system tuning is declared by the service in its own `/_info` response. lucos_monitoring holds no system-specific logic — it only owns generic cross-cutting probes (`fetch-info`, `tls-certificate`, `circleci`). Bit me 2026-05-05 when I proposed raising a failThreshold issue against lucos_monitoring for the media_manager `empty-queue` flap; lucas42 redirected it to the service repo.
