...
- [Incremental rsync path: TWO serial bugs from the photos seed](pattern_incremental_rsync_container_proxyjump_hostkey.md) — `incremental` backup (#324) had (1) #327 host-key on ProxyJump hop (empty container known_hosts) — **FIXED #329/v1.1.14** by mounting host known_hosts; (2) #330 publish step `rm && mv` sends UNQUOTED to ssh → `&&` runs on LOCAL shell so `mv` runs on avalon not aurora (tell: Irish-locale mv error) → data stranded in `.partial` — **OPEN**. create-backups red for photos_photos is now **#330** not #327; NOT new outage; no data at risk. Both block lucos_photos#427. Lesson: uncexercised-in-CI backup path hides bugs in SERIES.
