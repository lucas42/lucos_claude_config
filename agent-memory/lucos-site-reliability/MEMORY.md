...
- [#311 scp→rsync swap shipped without rsync in the Alpine image](pattern_backups_rsync_binary_missing_from_image.md) — `rsync: not found` → `backupToAll failed for N hosts` on EVERY volume (estate-wide off-host copy down, not just #309's). Dockerfile `apk add sed curl openssh-client` lacks rsync. Local tars still ok. Caught 2026-06-08 by ad-hoc run, not /_info. Fix: add rsync to apk line. Remote ends xwing/salvare have it; aurora unverified.
