...
- ⚠️ **`Audit dry-run` job sitting 20-30min on "Run dry-run audit sweep" is NOT stuck — do NOT cancel.** It blocks on a GitHub rate-limit reset by design (`auditDryRunMaxWaitDefault = 30 * time.Minute`, `src/cmd_audit.go`, from #462). Cancel+rerun restarts the wait AND spends more of the exhausted quota — strictly slower. It resumes at the reset and then needs ~6-8min of real work. Confirmed 2026-08-23: two runs blocked ~28-34min, both finished within 5s of each other after the reset, full correct diff (1420 unchanged / 0 new).
- ⚠️ **One merge = TWO full estate sweeps (~2,760 requests each) in one quota hour** → primary-quota exhaustion. The PR's dry-run sweeps, then the deploy restarts the container and `AuditSweeper.Start()` (`src/audit.go`) fires an immediate second sweep. 2026-08-23: exhausted installation 114293989; **production** sweep FAILED (`sweep incomplete: 74 convention check(s) skipped`) because its ceiling is `rateLimitMaxWait = 5 * time.Minute` (`src/ratelimit.go`) vs the dry-run's 30min. Disposable path tolerant, authoritative path brittle. Raised #497; #465 (open, design pinned) is the tolerance half.
- ⚠️ **A FAILED production sweep is SILENT** — no alert, `audit` check stays healthy, `seconds_since_last_sweep` = `-1` (sentinel: none completed *in this process*, resets on restart), next attempt `sweepInterval` = 6h. Estate audit data can be 6h stale behind a green dashboard. To check for real: `docker logs lucos_repos_app | grep "Audit sweep"` on avalon. Container is `lucos_repos_app`, not `lucos_repos`.
- ⚠️ **`conventions.SetHTTPClient()` is a PACKAGE GLOBAL shared by the sweep and the live `/api/audit` path.** `AuditSweeper.sweep()` installs its client (`src/audit.go`) for the sweep's whole duration; `src/audit_api.go` sets none, so live requests inherit the sweep's `MaxWait`. With the 30m ceiling that means a live request *waits* and nginx 504s it at ~60s (`curl -sf` → exit 22, body discarded). Its own doc comment says "not safe for concurrent use". #501. **Diagnostic:** `seconds_since_last_sweep == -1` ⇒ sweep in flight ⇒ global contaminated; positive ⇒ clear.
- **Primary quota is HOURLY, so a required rate-limit wait spans 0-60min** depending on where in the hour exhaustion lands — not the "~20-25min typical" in #462's comment. 2026-08-23: observed a genuine `wait=30m0s` (reset 11:02:34→12:02:34), within ~1s of the 30m ceiling being too small. Size ceilings against the ~60m bound, not the typical. Verify a logged wait is real, not the ceiling clamping the line, by checking it lands on `previous_reset + 1h`.
- **A sweep can park 19s in and hold the global for its whole wait** — so "sweep takes ~17min" is not the contaminated-window length; it can be 45min+.
