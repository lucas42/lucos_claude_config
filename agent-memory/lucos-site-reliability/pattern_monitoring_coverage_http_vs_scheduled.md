...
## Canonical per-host-agent template: lucos_docker_health (for cross-host monitoring)

`lucos_docker_health` is the reference pattern for "an agent deployed on EVERY host that reports per-host into monitoring": a tiny Go container (`hosts: [avalon,salvare,xwing]`, `http_port: null`, mounts `/var/run/docker.sock:ro`), which on a `REPORT_FREQUENCY` timer POSTs `statusReport` JSON `{system, job_name, frequency, status: success|error, message}` to `SCHEDULE_TRACKER_ENDPOINT`. **`job_name = HOSTDOMAIN first label`** (`avalon`/`xwing-v4`/`salvare-v4`) — that's how the per-host checks surface as distinct `(system, job_name)` entries. http_port is null precisely because it pushes, never serves /_info. **Reuse this mold for any new cross-host capability** rather than giving monitoring SSH-out or pull-probing. Applied to the lucas42/lucos_monitoring#285 perimeter-prober design (off-host firewall-bypass detection): per-host prober probes a peer's canary port + an anchor public_port, pushes refused/reachable/inconclusive via this same channel; monitoring needs zero code change. To run two reporters against one logical target without colliding on the `(system, job_name)` key, encode the vantage in job_name (`perimeter-avalon-from-xwing`).
