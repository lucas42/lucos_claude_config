...
**⚠️ DETERMINISTIC STARVATION (confirmed 2026-08-07, evidence on #173):** `import.py` walks `sorted(os.listdir(dirpath))` and Python sorts **uppercase before lowercase**, so the run always dies at the case boundary. Checkpoint on 2026-08-07 showed 15 capitalised dirs DONE and **11 lowercase dirs NEVER reached** (`artists`, `bandcamp`, `classical`, `comedy`, `compliations`, `compositions`, `iTunes`, `mixes`, `newgrounds`, `qobuz`, `songbird`, `symphony of science`). Same tail starved every week — not bad luck. **Consequence: the full scan is NOT a usable backstop for those dirs**, so a `new_files.py` miss there ([[pattern_new_files_unguarded_loop_poison_file]]) is permanent and needs a manual targeted re-import. Never claim "the weekly scan will catch it" without diffing `completed_dirs` against the sorted top-level listing.
