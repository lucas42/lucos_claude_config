...
## The render is UNCONSUMED — ~18% of a core for nothing (2026-08-09)

`monitoring_state_server.erl:201-208` — *"Always publish an SSE event on every updateSystem cast"* → `Publish(SystemList)`. `stream_handler:make_publish_fun/0` **renders first, then fans out**: `Html = view:render_dashboard_block(SystemList), stream_handler ! {publish, Html}` — nothing consults the subscriber list before the render. Measured: **0 SSE subscribers** (5 samples/50s, counting procs whose `current_function` is `{stream_handler, subscriber_loop, _}`) and only **2 `GET /event-stream` in 14.7h**. ⚠️`poll_timing` casts do **NOT** render (`:237` returns without `Publish`) — cast counts including them over-count renders ~2x.

**Duty cycle, measured directly** — sample the state server's `current_function` every 50ms, tally by module: `gen_server` 81.8% (idle in receive), **`re` 11.4%**, `lists` 6.2%, `erlang` 0.6% ⇒ **18.2% non-idle ≈ 10.9s per 60s cycle**. `re:` occurs at 5 sites in `view.erl` and **0** in the state server, so it is unambiguously render. **Cross-check: 105ms × ~100 updateSystem casts = 10.5s vs 10.9s measured — 3.7% apart, two methods, no shared inputs.**

⚠️**Why the architect's original corroboration (2.4s computed ≈ 3s observed) held for the WRONG reason** — two errors cancelling: per-render cost was 4.2x low, AND it was compared against my **burst width** (time the mailbox is *backlogged*, 8.7%) rather than **busy time** (18.2%). Different quantities: casts arriving spread out keep the server busy without ever building depth. **Generalises: agreement between a computed and an observed number means nothing until you check they measure the same quantity.** Conclusion survived, evidence didn't.

**Cheapest remedy (offered on #298, not pushed — architect owns it):** move the `view:` call inside `stream_handler`'s `{publish, ...}` branch, pass `SystemList` not `Html`, guard on `AliveClients =/= []`. Removes ~18% of a core and most of the burst; smaller than ETS/sharding.
