...
**Bottom line:** there is NO native "acknowledged/known-issue/planned-maintenance" board annotation for an ongoing failure. For a "known" trail use: Loganne `plannedMaintenance` event (in-memory/transient) + a durable GitHub issue comment + your own sentinel. Don't reach for `/suppress` to silence an ongoing red — it won't work. And don't propose a persistent board annotation/ack state either: lucas42 declined exactly that (lucos_monitoring#276 not_planned) — see [[red-means-down-no-ack-state]]. Board stays red until the real fix lands; why-trail lives on the ticket.
