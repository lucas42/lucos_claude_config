...
**Opening a PR is not "in review" or "open for review".** Those phrases are only accurate once Step 1 below has actually been executed — a message sent to `lucos-code-reviewer` and, ideally, some response received. **Loop-start is not observable on GitHub at all** — it is the SendMessage to `lucos-code-reviewer`, so the only agent who can attest to it is the one who sent it. Do **not** reach for `review_requested` timeline events as evidence either way: this workflow engages the reviewer by message, never by GitHub's request-review mechanism, so that count is `0` on every PR including ones that completed a full CHANGES_REQUESTED - APPROVED cycle. Reading it as "the loop never started" is a false negative on a fully-reviewed PR. The one GitHub-observable signal is a *submitted* review at `/pulls/{n}/reviews`, which tells you the reviewer has already responded — not whether they were asked. Before describing a PR's status to anyone — in a report, a comment, a memory note — check which state you're actually in and say so precisely: "opened, reviewer not yet messaged" is a different fact from "reviewer messaged, awaiting response" or "approved, awaiting merge".
...
- **Check supervision status**: run `~/sandboxes/lucos_agent/check-unsupervised {repo}` (exit 0 = unsupervised, exit 1 = supervised).
  - **Unsupervised repo**: do NOT re-request lucas42. The code-reviewer's approval triggers auto-merge. Never add lucas42 to the reviewer list — it puts a pointless request in his queue for a PR that has already or will auto-merge without him. Your job is done once the PR is approved or merged.
  - **Supervised repo**: check whether lucas42 has a pending CHANGES_REQUESTED (query `pulls/{n}/reviews`). If so, re-request his review now — the code reviewer's approval is the gate before putting the PR back in lucas42's queue. Use the developer app (not the coordinator, which lacks `pull_requests: write`):
    ```bash
...
Do not report the supervision status to the coordinator — they run `check-unsupervised` independently. Do not wait for CI, do not poll CI status.
...
**Before pushing a fix in response to a late-arriving instruction (coordinator message, lucas42 decision relayed asynchronously), re-fetch `pulls/{n}` and check `merged`.** Async messages can cross with a fast merge — a supervised-repo PR can go from "awaiting approval" to merged in the time it takes you to edit a file and stage a commit. Pushing to a branch whose PR already merged does not reopen it or add the commit to `main` — it silently orphans the branch (diverged from main, no open PR to carry it). If `merged: true`, cut a fresh branch from current `main`, carry just the intended change, open a new PR, and clean up the orphaned branch (`DELETE /git/refs/heads/{branch}`) rather than continuing to push to the dead one.
...
**Important: this also applies when you push changes to an already-approved PR** (e.g. a rework requested by the coordinator) **and when the change was requested by lucas42 rather than the code-reviewer** (the common case on a supervised repo where lucas42 tested and hit a bug). Pushing a new commit to a PR dismisses any prior approval and resets `review_decision` to null. **lucas42's CHANGES_REQUESTED does NOT mean "fix and re-request only lucas42":** the code-reviewer's earlier APPROVE is now stale on the new head, so the fix still needs a fresh code-review before it goes back to lucas42. The sequence is the same regardless of who requested the change — fix → code-reviewer (fresh review of the *new* commit) → approves → re-request lucas42. After every push — including reworks and lucas42-requested fixes — you must go back to step 1 before reporting back to the coordinator. Check that `review_decision` is not null and `mergeable_state` is not `blocked` on the new head commit before declaring the loop complete. Reporting "done" on a PR with `review_decision: null` is a failure to complete the loop.
