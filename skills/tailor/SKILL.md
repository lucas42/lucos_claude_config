...
- **Workday / iCIMS / Taleo / generic**: no public API. Ask Luke to paste a screenshot of the application form or list the fields by name. Don't guess. **Workday nuance:** the cover-letter upload commonly sits *inside* the same attachments step as the Resume/CV ("if you would like to attach a cover letter please do so in the section below"), NOT as a separate step. A Workday step-list with no "Cover Letter" step therefore does NOT mean the form is cover-letter-less — ask Luke specifically what the Resume/CV attachments step shows before settling the artefact set to CV-only. (Learned 2026-06-03: initially set CV-only off a 7-step list, then a cover-letter upload turned up in the Resume/CV step.)
