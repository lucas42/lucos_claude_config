...
**If no argument is provided, default to the top of the spotted queue.** Read `~/sandboxes/lukeblaney_cv_tailored/applications/spotted.md` and take the **first role entry** — the first `###` heading from the top of the file, which is the highest-priority role (the list is priority-ordered; the top-tier's first entry leads). Use that entry's **JD:** URL as the argument for the rest of the flow. **State which role you picked AND its JD URL** before proceeding ("No URL given — tailoring for the top of the queue: {Company} — {Role} — {JD URL}") so Luke can redirect if he meant a different one *and* open the JD in a separate tab to follow along (he asked for the URL up front, 2026-06-08 — don't make him reconstruct it from the job title). Don't make it a blocking gate, just an announced default he can interrupt.
...
- **Phenom** (`careers.{company}.com`, often `#LI-` tagged): WebFetch returns a templated "this job has been filled" message and the JSON API is tenant-gated — that message is **not** reliable evidence the role is closed. Don't treat it as filled. **The full JD is usually embedded in the raw page HTML as a JSON `description` field** (HTML-escaped), so fetch the page with `curl` (browser User-Agent) and extract it before falling back to Luke. Confirmed 2026-06-08 on a Phenom `careers.{company}` tenant:
  ```bash
  curl -s -m 20 -A "Mozilla/5.0" "<phenom-job-url>" -o /tmp/job.html
  python3 - <<'EOF'
  import re, html
  raw = open('/tmp/job.html', encoding='utf-8', errors='replace').read()
  m = re.search(r'"description":"(<p>.*?)","', raw, re.DOTALL)   # the unescaped-HTML description
  t = m.group(1).encode().decode('unicode_escape', errors='replace')
  t = re.sub(r'<li>', '\n- ', re.sub(r'</p>', '\n\n', t))
  print(html.unescape(re.sub(r'<[^>]+>', '', t)).strip())
  EOF
  ```
  The page also carries `"expired"` strings as template boilerplate — ignore them; they are not a closed signal. Only if the `description` field genuinely isn't present do you give Luke the URL and ask him to paste the JD.
...
- **Phenom (form-probe)**: form fields are JS-rendered, so ask Luke what the form shows (same as Workday/iCIMS/Taleo). **Phenom form shapes vary widely between roles even at the same tenant — never carry one role's shape over to the next.** Two roles in two days (2026-06): one form was CV-only + a profile-level "About you" box (no cover-letter field), while another was a "resume" upload + a separate **"additional attachments"** upload (which IS the cover-letter slot, same as the Workday attachments nuance below). So apply the Workday attachments rule to Phenom too: an "additional attachments / supporting documents" field is a cover-letter home — don't conclude CV-only without reading the actual field labels.
- **Workday / iCIMS / Taleo / generic**: no public API. Ask Luke to paste a screenshot of the application form or list the fields by name. Don't guess. **Workday nuance:** the cover-letter upload commonly sits *inside* the same attachments step as the Resume/CV (e.g. an upload field labelled "Resume / Covering Letter / any Other Supporting Documents"), NOT as a separate step. The generic Workday step-list ("Autofill with Resume → My Information → My Experience → Application Questions → …") is **identical across tenants and tells you nothing about the artefact set** — the attachments field label only surfaces on the actual form, often behind account creation. So: **never settle on CV-only from the step names, and never carry a prior company's CV-only outcome over to a new one** — the deciding evidence is the *upload field's label*, which only Luke can read once he reaches that step. Until he confirms it, treat the artefact set as provisional: build the CV (always needed) and tell Luke to read the upload-field label when he hits it; if it mentions a cover letter or supporting documents, draft the letter then (the positioning from Steps 4–8 is already settled, so it's a fast turnaround). (Learned 2026-06-03, then again 2026-06-04: twice concluded CV-only from a 7-step list; both times the attachments field actually accepted a cover letter.)
...
- **Disjoint content**: when multiple letter-shaped artefacts go in the same submission (e.g. "Why X?" + "Additional Information", or a cover letter + a "why you're a great match" textarea), re-verify the content split decided in Step 3.5 actually holds in the drafts. List the key facts / framings each artefact contains; flag any that appear in more than one and decide which artefact owns it.
- **No competing superlatives** (the failure mode the disjoint split itself creates): once you've given each surface a *different* lead story, make sure they don't each claim primacy for it. Two surfaces read together must not both say "the clearest/most direct/strongest parallel [or example] is X" about different X's — that reads as careless. Let at most ONE surface make the primacy claim (give it to the most role-relevant evidence); the other surface frames its lead as additional breadth ("beyond that…", "I've also…"), not a competing headline. Grep each prose artefact for superlatives (`clearest`, `most direct`, `strongest`, `best example`, `above all`) and reconcile.
