...
**Fix the whole artifact, not the clause you came to fix.** A title is usually corrected because someone named one defect in it — a stale count, a disproven cause. Patching just that clause and stopping is the trap: the title now *looks* reviewed, so nobody re-reads it, and any other error in it is preserved with a fresh timestamp. Re-read the entire title and body against the current understanding before moving on, and check the two agree with each other afterwards. Suspect the **leading** clause hardest: it is read first, it is the part a reader takes away, and it is the part most likely to encode the framing that has just been retired. Prefer deleting a figure over updating it when the figure will move again on the next occurrence — counts belong in the evidence table, not the headline.
