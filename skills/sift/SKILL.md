...
- **A hard, unmeetable cert/qualification gate caps the tier — keep it out of top.** Per `feedback_cert_gate_downweight.md` (Luke 2026-06-23, from the BBB Platform Engineering Director CV-screen reject): a role whose JD names an *essential* certification or formal qualification Luke does not hold and can't acquire before applying (ITIL, SAFe/Agile certs, CISSP-as-hard-gate, named-degree gates) is a structurally lower-yield bet and must NOT take top-tier tailoring effort. A JD **"or equivalent experience"** softener reduces but does **not** remove the down-weight — the equivalent-experience framing reads well to a human but an ATS/keyword filter may still gate on the literal cert. **Desirable/preferred** certs are not a gate — no down-weight. This is a tier-cap, not an exclusion: a strong equivalent-experience case can still justify applying, just not at top priority. Note the gate in the entry's tier-rationale. (Distinct from genuine *skills* gaps — named-compliance-framework / OT-ICS / VMware-Azure-years — which already pull a role down in Step 4.)
