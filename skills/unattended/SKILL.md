...
| 1b | **Ready is substantively met, not just set** | The body still presents an unresolved fork — an options list with none marked chosen ("suggested fixes (any/combination)", "three approaches", an unpicked A-vs-B) — and no comment settles it. The board field is a claim by whoever set it, and tickets are boarded Ready in error. Dispatching one wastes the implementer's turn and hands them a design decision that belongs at triage. **Read the comments before concluding this** — a fork is routinely settled in a comment and never folded back into the body; only an unresolved-in-body-*and*-comments fork holds. Report it for re-triage; never resolve the fork yourself to make it dispatchable. |
